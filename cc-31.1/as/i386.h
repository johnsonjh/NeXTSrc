...
#define MAX_MEMORY_OPERANDS 1	/* max memory ref per insn */
...
#define Imm16   0x10		/* 16 bit immediate */
#define Imm32   0x20		/* 32 bit immediate */
#define Imm1    0x40    	/* 1 bit immediate */
#define ImmUnknown Imm32	/* for unknown expressions */
#define Imm     (Imm8|Imm16|Imm32)    /* gen'l immediate */
/* memory */
...
#define SMALLEST_DISP_TYPE(num) \
  FITS_IN_SIGNED_BYTE(num) ? (Disp|Abs) : \
  FITS_IN_SIGNED_WORD(num) ? (Disp16|Disp32|Abs16|Abs32) : (Disp32|Abs32)

#define SMALLEST_IMM_TYPE(num) \
  (num == 1) ? (Imm1|Imm8|Imm16|Imm32) : \
  FITS_IN_SIGNED_BYTE(num) ? (Imm8|Imm16|Imm32) : \
  FITS_IN_SIGNED_WORD(num) ? (Imm16|Imm32) : (Imm32)
...
#define movextendKludge 0x10000
