     ...
     int *p;
{
  ...
  switch (TREE_CODE (t))
	...
	OB_PUTC (')');
	if (TREE_READONLY (TREE_TYPE (TREE_VALUE (TYPE_ARG_TYPES (t)))))
	  OB_PUTS ("const ");
	if (TREE_VOLATILE (TREE_TYPE (TREE_VALUE (TYPE_ARG_TYPES (t)))))
	  OB_PUTS ("volatile ");
	dump_type_suffix (TREE_TYPE (t), p);
	return;
      }
     ...
     tree cname, field;
{
  ...
  if (TREE_CODE (fntype) == METHOD_TYPE)
    {
      if (TREE_READONLY (TREE_TYPE (TREE_VALUE (TYPE_ARG_TYPES (fntype)))))
	OB_PUTS (" const ");
      if (TREE_VOLATILE (TREE_TYPE (TREE_VALUE (TYPE_ARG_TYPES (fntype)))))
	OB_PUTS (" volatile ");
    }
  {
