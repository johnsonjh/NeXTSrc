...
{
    register double v;
    asm("fsinx %1,%0": "=f" (v): "f" (x));
    return v;
}
double cos(register double x)
{
    register double v;
    asm("fcosx %1,%0": "=f" (v): "f" (x));
    return v;
}
double tan(register double x)
{
    register double v;
    asm("ftanx %1,%0": "=f" (v): "f" (x));
    return v;
}
#else
/* SIN(X), COS(X), TAN(X)
...
#else   /* IEEE double  */
#ifdef	NeXT
#define thresh (2.6117239648121182150E-1) /*Hex  2^ -2   *  1.0B70C6D604DD4 */
#define PIo4   (7.8539816339744827900E-1) /*Hex  2^ -1   *  1.921FB54442D18 */
#define PIo2   (1.5707963267948965580E0)  /*Hex  2^  0   *  1.921FB54442D18 */
#define PI3o4  (2.3561944901923448370E0)  /*Hex  2^  1   *  1.2D97C7F3321D2 */
#define PI     (3.1415926535897931160E0)  /*Hex  2^  1   *  1.921FB54442D18 */
#define PI2    (6.2831853071795862320E0)  /*Hex  2^  2   *  1.921FB54442D18 */
#define zero   (0)
#define one    (1)
#define negone (-1)
#define half   (1.0/2.0)
#define small  (1E-10)
#define big    (1E20)
/* 1+small**2==1; better values for small:    = 1.5E-9 for VAX D
...
#else	/* IEEE double  */
#ifdef	NeXT
#define S0  (-1.6666666666666463126E-1)  /*Hex  2^ -3   * -1.555555555550C */
#define S1   (8.3333333332992771264E-3)  /*Hex  2^ -7   *  1.111111110C461 */
#define S2  (-1.9841269816180999116E-4)  /*Hex  2^-13   * -1.A01A019746345 */
#define S3   (2.7557309793219876880E-6)  /*Hex  2^-19   *  1.71DE3209CDCD9 */
#define S4  (-2.5050225177523807003E-8)  /*Hex  2^-26   * -1.AE5C0E319A4EF */
#define S5   (1.5868926979889205164E-10) /*Hex  2^-33   *  1.5CF61DF672B13 */
#else
static double
...
#else	/* IEEE double  */
#ifdef	NeXT
#define C0   (4.1666666666666504759E-2)  /*Hex  2^ -5   *  1.555555555553E */
#define C1  (-1.3888888888865301516E-3)  /*Hex  2^-10   * -1.6C16C16C14199 */
#define C2   (2.4801587269650015769E-5)  /*Hex  2^-16   *  1.A01A01971CAEB */
#define C3  (-2.7557304623183959811E-7)  /*Hex  2^-22   * -1.27E4F1314AD1A */
#define C4   (2.0873958177697780076E-9)  /*Hex  2^-29   *  1.1EE3B60DDDC8C */
#define C5  (-1.1250289076471311557E-11) /*Hex  2^-37   * -1.8BD5986B2A52E */
#else
static double
