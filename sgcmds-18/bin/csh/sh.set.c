 ...
 * 	Modified doset to run dosetupterm() when the user types
 *	"set term = <val>".  This is going to make lots of people 
 *	happy!
 *
 * 29-Aug-85  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Export SHELL to environment when shell is set.
 *
 * 28-Aug-85  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Only call dohash() if havhash.
 *
 * 05-Jun-85  Steven Shafer (sas) at Carnegie-Mellon University
 *	Added dobindings() call when user changes "editmode" variable;
 *	also put into EDITMODE environment parameter.
 *
 **********************************************************************
 */
...
setsyn:
...
#if	CMUCS
		{
			...
			setenv("SHELL", value(vp));
		else if (eq(vp, "editmode")) {
			setenv("EDITMODE", value(vp));
			if (!cmdedit || eq(value(vp), "none")) {
				if (!tenex_initialized())
					cmdedit = 0;
				continue;
			}
			dobindings(value(vp));
