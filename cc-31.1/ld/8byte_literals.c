...
struct section_map *section_map)
{
	 ...
	 */
	if(s->size % 8 != 0){
	    error_with_cur_obj("8 byte literal section (%0.16s,%0.16s) size is "
...
struct merged_section *ms)
{
	...
	while(i < ms->order_size){
		...
		if(get_hex_from_sectorder(ms, &i, &(literal8.long1),
		    ...
		    error("format error in -sectorder file: %s line %d for "
			  "section (%0.16s,%0.16s) (missing second hex number)",
...
long line_number)
{
	...
	if(i > ms->order_size || ms->order_addr[i] != '0'){
	    error("format error in -sectorder file: %s line %d for section "
		  "(%0.16s,%0.16s) (missing hex number, no leading 0x found)",
	...
	if(j > 8){
	    error("format error in -sectorder file: %s line %d for section "
		  "(%0.16s,%0.16s) (too many hex digits for 32 bit value)",
		  ms->order_filename,line_number,ms->s.segname,ms->s.sectname);
	    *index = i;
	    return(FALSE);
	}
...
struct merged_section *ms)
{
    ...
    long align_multiplier, output_offset, i;
...
struct merged_section *ms)
{
    long align_multiplier, i, offset;
    struct literal8_block **p, *literal8_block;
...
char *indent)
{
	...
	if(data == NULL)
	    return;
	print("%s   literal8_blocks 0x%x\n", indent, data->literal8_blocks);
	for(p = &(data->literal8_blocks); *p ; p = &(literal8_block->next)){
	    literal8_block = *p;
	    print("%s\tused %d\n", indent, literal8_block->used);
	    print("%s\tnext 0x%x\n", indent, literal8_block->next);
	    print("%s\tliteral8s\n", indent);
	    for(i = 0; i < literal8_block->used; i++){
		print("%s\t    0x%08x 0x%08x\n", indent,
		      literal8_block->literal8s[i].long0,
		      literal8_block->literal8s[i].long1);
	    }
	}
}
...
struct merged_section *ms)
{
	if(data == NULL)
	    return;
	print("literal8 section (%0.16s,%0.16s) contains:\n",
	      ms->s.segname, ms->s.sectname);
	print("    %d merged literals \n", ms->s.size / 8);
	print("    from %d files and %d total literals from those "
