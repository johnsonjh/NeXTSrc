...
static char sccsid[] = 	"@(#)svcauth_des.c	1.9 88/08/02 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	register int *groups;
{
	...
	if (sid >= AUTHDES_CACHESZ) {
#ifdef	KERNEL
		printf("authdes_getucred:  invalid nickname");
#else
	...
	if (cred->grouplen == INVALID) {
		...
		{
#ifdef        KERNEL
			printf("authdes_getucred:  unknown netname");
#else
...
#ifdef        KERNEL
		printf("authdes_getucred:  missed ucred cache");
#else
