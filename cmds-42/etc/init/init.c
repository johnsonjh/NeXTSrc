...
main(argc, argv)
	char **argv;
{
	volatile int howto, oldhowto;

	time0 = time(0);

#ifdef DEBUG
	filep = fopen ("/tmp/initlog", "w");
#endif DEBUG	
		...
		if (pid == 0) {
			...
			if (passwd != (char *)NULL)  {
				do {
					...
					if (strncmp (try, passwd, NVRAM_HW_PASSWD)
					    == 0)  {
...
multiple()
{
	...
	for (EVER) {
		...
		pid = wait(&status);
#ifdef DEBUG
		fprintf (filep, "MULTIPLE: pid (%d) p.pid (%d) p.wpid(%d)", 
			pid, p->pid, p->wpid);
#endif DEBUG
#else	NeXT_MOD
		pid = wait((int *)0);
...
merge()
{
	...
	while (t = getttyent()) {
		...
		if (!p1) {
			fprintf(filep, "no space for '%s' !?!", t->ty_name);
			goto contin1;
		}
		/*
	...
	for (ALL) {
		if ((p->xflag&FOUND) == 0) {
#ifdef DEBUG
			fprintf (filep, "FOUND term() wterm()");
#endif DEBUG
			term(p);
			wterm(p);
			if (p1)
...
#ifdef	NeXT_MOD
#ifdef DEBUG
				fprintf (filep, "CHANGE dfork COMMON");
#endif DEBUG
				dfork(p, COMMON);
#else	NeXT_MOD
				dfork(p);
...
#endif	NeXT_MOD
{
	...
	if (pid == 0) {
		...
		if (dowait) {
			fprintf(filep, "'%s %s' failing, sleeping", p->comn, p->line);
			closelog();
			sleep(30);
		}
#ifdef	NeXT_MOD
		switch (which)  {
		    case COMMON:
#ifdef DEBUG
			fprintf (filep, "COMMON");
#endif DEBUG
			execit(p->comn, p->line);
			break;
		    case ONOPTION:
#ifdef DEBUG
			fprintf (filep, "ONOPTION");
#endif DEBUG
			if (p->ocmd[0])  
	...
	register struct tab *p;
{
	...
	if (f >= 0) {
		...
		if (utmpsize < statbf.st_size) {
			...
			if (!utmp)
				fprintf(filep, "utmp malloc failed");
		}
	...
	register struct tab *p;
{
	...
	if (pid == 0) {
		...
		sigsetmask(0);	/* since can be called from masked code */
		if (dowait) {
			fprintf(filep, "'%s %s' failing, sleeping", p->wcmd, p->line);
			closelog();
			sleep(30);
		}
#ifdef DEBUG
		fprintf (filep, "WSTART");
#endif DEBUG
		execit(p->wcmd, p->line);
...
#ifdef DEBUG
	fprintf (filep, ":execit: %s %s", s, arg);
#endif DEBUG
	/*
