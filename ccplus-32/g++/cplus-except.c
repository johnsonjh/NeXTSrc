...
/* Cache `setjmp', `longjmp', and `abort' after default conversion.
   Maybe later they will get built-in.  */
static tree BISJ, BILJ, BIA;
     ...
     int keep;
{
  ...
  /* Pops binding contour local to TRY.  */
  if (keep)
    {
...
/* Note that this must be mirror image of `...start_try'.
   ...
   EXITFLAG is nonzero if we should try an outer handler.  */
void
cplus_expand_end_except (dfault, exitflag)
     tree dfault;
     int exitflag;
{
  ...
  if (dfault == NULL_TREE)
      ...
      if (in_try_block (1) == 0)
	{
	  ...
	  call_to_abort = build_function_call (BIA, NULL_TREE);
	  expand_expr (call_to_abort, 0, VOIDmode, 0);
	  expand_end_cond ();
	}
    }
  if (exitflag)
  ...
  if (! in_try_block (1))
  ...
  else if (dfault == NULL_TREE)
    {
	 ...
	 The exit_label will be the beginning of the next try block.  */
      if (! expand_escape_except ())
	compiler_error ("except nesting botch");
      expand_end_cond ();
    }
...
void
expand_cplus_raise (raise_id, parms, exp)
     tree raise_id;
     tree parms;
     tree exp;
{
...
void
expand_cplus_start_catch (raise_id)
     tree raise_id;
{
...
void
expand_cplus_reraise (exceptions)
     tree exceptions;
{
  tree current_exception_ptr = build (NOP_EXPR, NULL_TREE,
  ...
  while (exceptions)
      ...
      if (type == NULL_TREE)
	{
	  ...
	  TREE_TYPE (current_exception_ptr) = error_mark_node;
	}
      else
	{
	  ...
	  TREE_TYPE (current_exception_ptr) = TYPE_POINTER_TO (type);
	}

      /* Now treat reraise like catch/raise.  */
      expand_cplus_start_catch (exceptions);
      expand_cplus_raise (exceptions, NULL_TREE, current_exception_ptr);
      /* Set to zero so that destructor will not be called.  */
      if (TREE_TYPE (current_exception_ptr) != error_mark_node)
	expand_assignment (current_exception_ptr, integer_zero_node, 0, 0);
      expand_cplus_end_catch (exceptions);
      exceptions = TREE_CHAIN (exceptions);
    }
  /* Don't propagate any unhandled exceptions.  */
  expand_expr (build_function_call (BIA, NULL_TREE), 0, VOIDmode, 0);
}

void
...
init_exception_processing ()
{
  ...
  BIA = default_conversion (abort_fndecl);
