	...
	char cc;
{
	...
	sprintf(line, "cat %s;echo \026\01", argv[0]);
	transfer(line, fd, "\01");
}

static struct buf_stuff {
	int bs_fd;
	char *bs_bp;
	char *bs_end;
	char bs_buf[BUFSIZ];
...
bufread(void)
{
	...
	if (bs.bs_bp >= bs.bs_end) {
		if ((cnt = read(bs.bs_fd, bs.bs_buf, sizeof(bs.bs_buf))) <= 0)
			return(-1);
		bs.bs_end = &bs.bs_buf[cnt];
		bs.bs_bp = bs.bs_buf;
	}
	return(*bs.bs_bp++ & 0177);
}
	...
	char *buf, *eofchars;
{
	...
	if (errno)
		oldpri = 0;
	setpriority(PRIO_PROCESS, 0, HIGHPRI);
#endif HIGHPRI
	lpmsg = number(value(BAUDRATE))/400;
	if (lpmsg == 0)
		lpmsg = 1;
	...
	for (ct = 0; !quit;) {
		...
		*p++ = c;
		...
		if (c == '\n' && boolean(value(VERBOSE)) && ++ct % lpmsg == 0)
			printf("\r%d", ct);
