 ...
 **********************************************************************
 * HISTORY
 * 13-Jan-87  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Added some minimal code to understand directories above the
 *	super-root.  Doesn't handle relative movement from one side
 *	of the super-root to the other very well in all cases.
 *
 **********************************************************************
 */
...
#if	NeXT
/*
 * CMUCS code for handling super-root does not work when no super-root.
 * (Try, cd /; cd ..; cd.. to see)
 */
#undef	CMUCS
#endif	NeXT
		...
		*p1++ = '/';
		...
		for (;;) {
			*p1 = '\0';
			...
			if (lastino == 0)
				p1--;
			else if (lastino == (int)statb.st_ino)
				break;
			lastino = (int)statb.st_ino;
			*p1++ = '/';
			*p1++ = '.';
			*p1++ = '.';
			rootdepth++;
