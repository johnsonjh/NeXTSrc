	...
	ADDRESS **sendq;
{
	register char *p;
#ifdef  NETINFO
	extern char *netinfolookup();
#endif  NETINFO
#ifdef	YELLOWPAGES
	extern char *yellowlookup();
#endif  YELLOWPAGES
...
#ifdef DEBUG
	syslog (LOG_INFO, "ALIAS: %s (%s, %s) aliased to %s\n",
...
# ifdef DEBUG
	if (tTd(27, 1))
		printf("ALIAS: %s (%s, %s) aliased to %s\n",
		    a->q_paddr, a->q_host, a->q_user, p);
# endif
...
**	Called from:
**		deliver.c::sendall() alias.c::alias()
*/
...
/*
** MAPUSERNAME -- look up a string in the NetInfo with replacement
**	       -- if NetInfo cannot find it, then try Yellow Pages
** 
**
**	Parameters:
...
/*
**  NETINFOLOOKUP -- Look up a name in NeXT's NetInfo
**
**	Author:
**		Morris Meyer (Please do not associate my name with sendmail)
**
**	Yellowlookup doesnt handle ":" and "|" (includes and pipe's)
**	respectively.  We are currently not handling ":" and "|" and
**	probably wont.
**
**	Parameters:
...
#ifdef NETINFO
char *netinfolookup(a)
    register ADDRESS *a;
{
	char *result, **memberptr, **p;
	int count, size, len;
	static struct aliasent *aptr;

	size = 0;
	aptr = alias_getbyname (a->q_user);
	if (aptr == NULL)  
	    	return (NULL);

	memberptr = aptr->alias_members;
	len =       aptr->alias_members_len;

	for (p = memberptr, count = 0; count < len; *p++, count++)  
	    	size += (strlen (*p) + 2);

	if ((result = (char *) calloc (1, size)) == (char *)NULL)
	    	return (NULL);
	...
	result [strlen (result) -1] = '\0';  /* Tramp on last comma */
