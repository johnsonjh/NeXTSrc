...
tperror() {
	...
	if (!query("Do you want to restart?"))
		dumpabort();
	if (optical)  {
		msg("This disk will eject.  After it ejects,\n");
...
#endif NeXT_MOD
{
...
#ifdef NeXT_MOD
	msg("%s\n", (optical ? "Disk ejecting" : "Tape rewinding"));
#else
	msg("Tape rewinding\n");
#endif NeXT_MOD
#ifdef RDUMP
	if (host) {
		...
		if (optical &&  (rmteject(tape) == -1))  {
			msg ("Error trying to eject disk");
