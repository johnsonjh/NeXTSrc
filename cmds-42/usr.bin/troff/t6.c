...
#ifdef NeXT_MOD
		/*
		 * No one at Bell thought there'd ever be a path with
		 * a 'X' in it!  (Draw your own conclusions....)
		 */
		{
			extern char *rindex();
			char *cp = rindex(fontfile, 'X');
			if (cp == (char *)0)
				ffi = strlen(fontfile);
			else {
				while (cp > fontfile && *(cp-1) == 'X')
					cp--;
				ffi = cp - fontfile;
			}
		}
