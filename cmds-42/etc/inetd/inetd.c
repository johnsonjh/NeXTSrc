...
#ifndef lint
static char sccsid[] = "@(#)inetd.c   1.2 88/03/10 4.0NFSSRC"; /* from 1.38 88/02/07 SMI; from UCB 5.6 8/19/86 */
#endif not lint
...
#if	NeXT
	/*
	 * Set our bootstrap port to a restricted port so that launched
	 * services can't pose as someone they're not.
	 */
	{
		kern_return_t r;
		
		r = bootstrap_get_unpriv_port(bootstrap_port, &bootstrap_port);
		if (r != KERN_SUCCESS) {
			mach_error("bootstrap_get_unpriv_port", r);
			exit(1);
		}
		r = task_set_bootstrap_port(task_self(), bootstrap_port);
		if (r != KERN_SUCCESS) {
	...
	register struct servtab *sep;
{
	...
	if (bind(sep->se_fd, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		syslog(LOG_ERR, "%s/%s: bind: %m",
		    sep->se_service, sep->se_proto);
		(void) close(sep->se_fd);
		sep->se_fd = -1;
	...
	if (getsockname(sep->se_fd, (struct sockaddr *)&addr, &len) != 0) {
