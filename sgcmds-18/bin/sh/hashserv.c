...
hashpr()
{
	prs_buff(hashhdr);	/* DAG -- made strings sharable */
	hscan(hashout);
}
	...
	if (h)
	{
		...
		switch (hashtype(h->data))
		{
			...
			case FUNCTION:
			{
				...
				prs_buff(isfunct);	/* DAG */
				prs_buff(name);
				prs_buff(sfuncstr);	/* DAG */
				prf(n->namenv);
				prs_buff(efnlstr);	/* DAG */
				return;
			}
			...
			case COMMAND:					
				prs_buff(ishashed);	/* DAG */
				pr_path(name, hashval);
				prs_buff(rpnlstr);	/* DAG */
				return;
		}
	}
