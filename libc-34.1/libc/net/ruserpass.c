/* @(#)ruserpass.c	1.1 87/07/08 3.2/4.3NFSSRC */
/*
...
#include <stdlib.h>
#include <string.h>
char	*renvlook(), *getenv(), *getpass(), *getlogin(), *ttyname();
#else
 ...
 */
typedef struct _ruserpassglobals {
	char tokval[100];
	char	E[48];
	/*
...
static ruserpassglobals_t *
getruserpassglobalsp(void)
{
	static ruserpassglobals_t *ruserpassglobalsp;

	if (!ruserpassglobalsp) {
		ruserpassglobalsp = calloc(1, sizeof(*ruserpassglobalsp));
	}
	return (ruserpassglobalsp);
}

#define tokval getruserpassglobalsp()->tokval
#define E getruserpassglobalsp()->E
#define C getruserpassglobalsp()->C
#define D getruserpassglobalsp()->D
#define KS getruserpassglobalsp()->KS
#define L getruserpassglobalsp()->L
#define R getruserpassglobalsp()->R
#define tempL getruserpassglobalsp()->tempL
#define f getruserpassglobalsp()->f
#define preS getruserpassglobalsp()->preS
