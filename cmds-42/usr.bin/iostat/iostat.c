...
int	ndrives = 0;
#ifdef vax
char	*defdrives[] = { "hp0", "hp1", "hp2", "hp3" };
#else
char	*defdrives[] = { 0 };
#endif
	...
	char *argv[];
{
	...
	if(nl[X_DK_BUSY].n_type == 0) {
		printf("dk_busy not found in /mach namelist\n");
		exit(1);
	}
	mf = open("/dev/kmem", 0);
	if(mf < 0) {
...
#if	NeXT
	dk_bps = (long *)calloc(dk_ndrive, sizeof (long));
#else	NeXT
...
#define	allocate(e, t) \
	...
	if (phz)
		hz = phz;
#if	NeXT
	lseek(mf, (long)nl[X_DK_BPS].n_value, L_SET);
	read(mf, dk_bps, dk_ndrive*sizeof (dk_bps));
#else	NeXT
...
stats(dn)
{
	...
	xtime = bytes / dk_bps[dn];	/* transfer time */	
#else	NeXT
	...
	if (itime < 0)
		xtime += itime, itime = 0;
#if	NeXT
	printf("%4.0f", bytes/1024/etime);	/* KB/sec */
#else	NeXT
...
read_names()
{
	...
	register struct bus_device *bdp;
#ifndef NeXT
	struct bus_driver bdr;
#endif NeXT
	short two_char;
	char *cp = (char *) &two_char;

	bdp = (struct bus_device *) nl[X_BUSDINIT].n_value;
	if (bdp == 0) {
	...
	if (bdp) for (;;) {
		...
		if (bd.bd_driver == 0)
			break;
		if (bd.bd_dk < 0 || bd.bd_alive == 0)
			continue;
#ifdef NeXT
		steal(bd.bd_name, two_char);
		sprintf(dr_name[bd.bd_dk], "%c%c%d",
		    cp[0], cp[1], bd.bd_unit);
#else
		steal(bd.bd_driver, bdr);
		steal(bdr.br_dname, two_char);
		sprintf(dr_name[bd.bd_dk], "%c%c%d",
		    cp[0], cp[1], bd.bd_unit);
#endif NeXT
	}
