	...
	char *argv[];
{
	...
	while ((n = getopt(argc, argv, "vsfF")) != EOF) {
		switch (n) {
		case 'v':
			verbose = 1;
			break;
		...
		case 'f':
			...
			break;
		case 'F':{
			/* let's find a host to set to in netinfo */
			whatHappened = ni_open(NULL, ".", &currentDomain);
			whatHappened = ni_pathsearch(currentDomain, &locDir, 
			"/locations/ntp");
			...
			if (whatHappened != NI_OK) {
				fprintf(stderr,"Problem obtaining NTP initialization data, aborting. Unable to set time\n");
			...
			if ((whatHappened == NI_OK) && currentProp.ni_namelist_len && 
				...
				if (sscanf(currentProp.ni_namelist_val[0], "%s", 
					&hostBuffer) != 1)
					fprintf(stderr, 
						"init_ntp: initialization error, server\n");
			if (*hostBuffer == 0) {
...
finishSet:
		hp = NULL;
#ifdef NeXT
		HostAddr = inet_addr(workingHost);
#else
