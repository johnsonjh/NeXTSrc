...
#ifndef lint
static char sccsid[] = "@(#)catman.c	1.2 88/06/18 NFSSRC4.0 from	5.7 (Berkeley) 5/27/86";
#endif not lint
...
char	buf[BUFSIZ];
char	uniqflag=1;	/* if true (default) pipe nroff through uniq */
char	pflag;
char	nflag;
char	wflag;
	...
	char *av[];
{
	...
	if (ac > 1) {
usage:
		printf("usage: catman [-p] [-n] [-w] [-u] [-M path] [ sections ]\n");
		exit(-1);
	}
	if (ac == 1)
		sections = *av;
		...
		*msp = *csp = '\0';
		...
		while ((dir = readdir(mdir)) != NULL) {
			...
			if (makelink) {
				...
				sprintf(buf, "nroff -man %s %s > %s", man, 
					uniqflag? "| uniq" : "", cat);
				if (pflag)
					printf("%s\n", buf);
