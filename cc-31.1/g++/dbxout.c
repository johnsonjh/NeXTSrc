...
/* At the end of compilation, finish writing the symbol table.
   Write an N_SO symbol with a null name to mark the end of the symtab.  */

void
dbxout_finish (asm_file, input_file_name)
     FILE *asm_file;
     char *input_file_name;
{
  fprintf (asm_file,
...
{
#ifdef NeXT
  /* Make local symbols use the letter 'l' rather than no letter at all.
     This makes parsing stabs easier and should make future extensions easier.
     GDB already knows how to grok this, but DBX probably doesn't. */
...
#ifdef NeXT
  /* Don't ouput stabs for Objective-C internal symbols. */
  if (!strncmp (IDENTIFIER_POINTER (DECL_NAME (decl)), "_OBJC_", 6))
    return;
#endif  /* NeXT */
...
#ifdef NeXT
  /* Since source line info will be written for all decls in dbxout_finish_symbol,
     we must make sure we are in the right source file before starting the stab */
    {
      extern char *lastfile;	/* From final.c */
  
      if (DECL_SOURCE_FILE (decl) &&
          (lastfile == 0 || strcmp (DECL_SOURCE_FILE (decl), lastfile)))
        {
#ifdef ASM_OUTPUT_SOURCE_FILENAME
          ASM_OUTPUT_SOURCE_FILENAME (file, DECL_SOURCE_FILE (decl));
#else  /* ASM_OUTPUT_SOURCE_FILENAME */
          fprintf (asmfile, ".stabs \"%s\",%d,0,0,Ltext\n",
	           DECL_SOURCE_FILE (decl), N_SOL);
#endif  /* ASM_OUTPUT_SOURCE_FILENAME */
          lastfile = DECL_SOURCE_FILE (decl);
        }
      
      current_sym_lineno = DECL_SOURCE_LINE (decl);
    }
#endif  /* NeXT */
...
{
#ifdef NeXT
  /* Output source line info for all decl's. */
  
  fprintf (asmfile, "\",%d,0,%d,", current_sym_code, current_sym_lineno);
  
  /* Make sure we use the deafult of zero next time unless someone changes it. */
  current_sym_lineno = 0;
#else  /* NeXT */
