	...
	int wanttty, *pipein, *pipeout;
{
	...
	case TPAR:
		...
		if (((t->t_dflg & FTIME) || (t->t_dflg & FPAR) == 0 &&
			...
			struct sigvec ointrvec, oquitvec, otstpvec;
			struct sigvec ottinvec, ottouvec, otermvec;
			struct sigvec ohupvec;

			sigvec(SIGINT, (struct sigvec *)0, &ointrvec);
			sigvec(SIGQUIT, (struct sigvec *)0, &oquitvec);
			sigvec(SIGTSTP, (struct sigvec *)0, &otstpvec);
			sigvec(SIGTTIN, (struct sigvec *)0, &ottinvec);
			sigvec(SIGTTOU, (struct sigvec *)0, &ottouvec);
			sigvec(SIGTERM, (struct sigvec *)0, &otermvec);
			sigvec(SIGHUP, (struct sigvec *)0, &ohupvec);
#endif sun || defined(NeXT)
...
#if defined(sun) || defined(NeXT)
				sigvec(SIGINT, &ointrvec, (struct sigvec *)0);
				sigvec(SIGQUIT, &oquitvec, (struct sigvec *)0);
				sigvec(SIGTSTP, &otstpvec, (struct sigvec *)0);
				sigvec(SIGTTIN, &ottinvec, (struct sigvec *)0);
				sigvec(SIGTTOU, &ottouvec, (struct sigvec *)0);
				sigvec(SIGTERM, &otermvec, (struct sigvec *)0);
				sigvec(SIGHUP, &ohupvec, (struct sigvec *)0);
#endif sun || NeXT
				xfree(Vsav); Vsav = 0;
				xfree(Vdp); Vdp = 0;
...
#ifdef NeXT_MOD
			/*
			 * These are necessary because globone()
			 * calls getpwnam() which creates garbage
			 * descriptors and steals the fd we just
			 * closed.  Another nasty problem is that
			 * the exec'ed child has trash fds beyond
			 * 0, 1, and 2; but that's too hard to
			 * fix just now.
			 */
			(void) close(0);
#endif NeXT_MOD
