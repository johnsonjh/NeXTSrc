/*	ex_cmdsub.c	1.2	85/12/10	*/
/* Copyright (c) 1981 Regents of the University of California */
static char *sccsid = "@(#)ex_cmdsub.c	7.2	10/31/81";
/* fixed tagfind() to use parent directory of tag file when
** 	searching for tag.  Mike Hawley, ucbvax!dagobah!mike, Dec 10, 1985
*/
#include "ex.h"
#include "ex_argv.h"
#include "ex_temp.h"
 ...
 * Fixed to find files in parent directory of tags file. (mjh)
 */
tagfind(quick)
	bool quick;
{
...
badtag:
	...
	while (fne) {
		...
		/* save pathname of tags file in dirbuf */
		{
		   char *rindex(), *s = rindex(strcpy(dirbuf,fn),'/');
		   if (s) *(s+1) = '\0';
		}
...
badtags:
			...
			*lp++ = 0;
			...
			/*
			** if filebuf doesn't exist, try prepending
			** current tagfile's pathname to it.
			*/
			if (*filebuf != '/' && access(filebuf,0)){
				char s[FNSIZE];
				strcpy(filebuf,sprintf(s,"%s%s",dirbuf,filebuf));
			}
