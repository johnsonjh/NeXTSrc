...
int tree_node_kinds[(int)all_kinds];
...
char *tree_node_kind_names[] = { "decls", "types", "stmts", "refs", "exprs", "constants",
				 ...
				 "random kinds", "lang_decl kinds", "lang_type kinds" };
...
init_tree ()
{
  ...
  obstack_init (&temp_decl_obstack);
     ...
     struct function *p;
{
  ...
  maybepermanent_firstobj = (char *) obstack_finish (&maybepermanent_obstack);
  temporary_firstobj = (char *) obstack_finish (&temporary_obstack);
...

#ifdef NeXT_STATISTICS
char *obstack_code_name[] =
{
  "Permanent",
  "Maybeperm",
  "Temporary",
...
int get_obstack_code (struct obstack *obstack)
{
  if (obstack == &permanent_obstack)
    return 0;
  if (obstack == &maybepermanent_obstack)
    return 1;
  if (obstack == &temporary_obstack)
    return 2;
  if (obstack == &momentary_obstack)
    return 3;
}

unsigned int tree_node_count[NUM_TREE_CODES][4];
unsigned int tree_node_size[NUM_TREE_CODES][4];
#endif  /* NeXT_STATISTICS */
     ...
     enum tree_code code;
{
  ...
  switch (type)
    {
#ifdef NeXT_OBJC
    case 'o':  /* An Objective-C node */
      if (code == KEYWORD_DECL)
	length = sizeof (struct objc_tree_keyword_decl);
      else if (code == INSTANCE_METHOD_DECL || code == CLASS_METHOD_DECL)
	length = sizeof (struct objc_tree_method_decl);
      else if (code == INTERFACE_TYPE || code == IMPLEMENTATION_TYPE ||
	       code == CATEGORY_TYPE || code == PROTOCOL_TYPE)
	length = sizeof (struct objc_tree_class_type);
	...
	+ tree_code_length[(int) code] * sizeof (char *);
      ...
      if (obstack != &permanent_obstack)
	obstack = saveable_obstack;
      /* PARM_DECLs always go on the saveable_obstack, not permanent
  ...
  tree_node_kinds[(int)kind]++;
  tree_node_sizes[(int)kind] += length;
#endif

#ifdef NeXT_STATISTICS
  tree_node_count[code][get_obstack_code (obstack)]++;
  tree_node_size[code][get_obstack_code (obstack)] += length;
#endif  /* NeXT_STATISTICS */
...
#ifdef NeXT_STATISTICS
  tree_node_count[code][get_obstack_code (current_obstack)]++;
  tree_node_size[code][get_obstack_code (current_obstack)] += length;
#endif  /* NeXT_STATISTICS */

#ifdef NeXT_PARANOIA
  /* Make sure we don't go beyond the space we malloc'ed. */
  bcopy (node, t, length);
#else  /* NeXT_PARANOIA */
...
#ifdef GATHER_STATISTICS
  tree_node_kinds[(int)x_kind]++;
  tree_node_sizes[(int)x_kind] += length;
#endif
...
#ifdef GATHER_STATISTICS
  tree_node_kinds[x_kind]--;
  tree_node_sizes[(int)x_kind] -= sizeof(struct tree_list);
  tree_node_kinds[perm_list_kind]++;
  tree_node_sizes[(int)perm_list_kind] += sizeof(struct tree_list);
#endif
  TREE_CHAIN (node) = chain;
  TREE_PURPOSE (node) = purpose;
  TREE_VALUE (node) = value;
...
#ifdef GATHER_STATISTICS
  tree_node_kinds[x_kind]--;
  tree_node_kinds[temp_list_kind]++;
  tree_node_sizes[(int)x_kind] -= sizeof(struct tree_list);
  tree_node_sizes[(int)temp_list_kind] += sizeof(struct tree_list);
#endif
  TREE_CHAIN (node) = chain;
  TREE_PURPOSE (node) = purpose;
  TREE_VALUE (node) = value;
     ...
     tree node;
{
  ...
  if (*tree_code_type[(int) code] == 'r')
    kind = r_kind;
  else if (*tree_code_type[(int) code] == 'e')
    kind = e_kind;
  else
    abort ();
...
/* That is not done, deliberately, so that having error_mark_node
  ...
  /* A freshly built node has these properties anyway.  */
#if 0
...
tree
build_loop (filename, line, vars, cond, body)
     char *filename;
     int line;
     tree vars, cond, body;
{
  ...
  STMT_SOURCE_FILE (t) = filename;
  STMT_SOURCE_LINE (t) = line;
  STMT_LOOP_VARS (t) = vars;
  STMT_LOOP_COND (t) = cond;
  STMT_LOOP_BODY (t) = body;
  return t;
}
     ...
     tree fndecl;
{
  ...
  while (context)
	...
	context = DECL_CONTEXT (context);
      else if (TREE_CODE (context) == LET_STMT)
...
dump_tree_statistics ()
{
  ...
  extern struct obstack class_obstack;
  ...
  for (i = 0; i < (int) all_kinds; i++)
    {
      fprintf (stderr, "%-20s %6d %9d\n", tree_node_kind_names[i],
	       tree_node_kinds[i], tree_node_sizes[i]);
      total_nodes += tree_node_kinds[i];
