...
static char sccsid[] = 	"@(#)auth_unix.c	1.4 88/07/27 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	int *aup_gids;
{
	...
	if (! xdr_authunix_parms(&xdrs, &aup)) {
		(void) syslog(LOG_ERR, "authunix_create:  xdr_authunix_parms failed");
		abort();
	}
...
authunix_create_default()
{
	...
	if (gethostname(machname, MAX_MACHINE_NAME) == -1) {
		(void) syslog(LOG_ERR, "authunix_create_default:  gethostname failed:  %m");
		abort();
	}
	...
	if ((len = getgroups(NGRPS, gids)) < 0) {
		(void) syslog(LOG_ERR, "authunix_create_default:  getgroups failed:  %m");
		abort();
	}
