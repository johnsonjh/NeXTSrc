...
/*
 * BE WARNED: Before you go and up these constants, be SURE that the values
 * you allow will fit into the variables that hold them!!  In particular,
 * if you increase the number of states, you'll have to change a number of
 * variables from unsigned char's to unsigned short's.  --Mike
 */
#define BLKSIZE 8192
#define MAXLIN 475
#define MAXPOS 6000
#define NSTATES 255		/* gotofn must be able to hold this! */

#define NCHARS 128		/* make sure CHARMASK matches */
			...
			do {
				...
				if (c == '-' && cclcnt > 0 && chars[nxtchar-1] != 0) {
					if ((d = nextch()) != 0) {
						c = chars[nxtchar-1];
						while (c < d) {
							if (nxtchar >= MAXLIN) overflo(LINOVF);
							chars[nxtchar++] = ++c;
							cclcnt++;
						}
						continue;
...
int kind;
{
	...
	fprintf(stderr, "egrep: %s exceeded\n", ovfmsg[kind]);
	exit(2);
}
...
add(array, n) int *array; {
	register i;
	if (nxtpos + count > MAXPOS) overflo(POSOVF);
	array[n] = nxtpos;
...
unsigned char *file;
{
	...
	if (buf == NULL) {
		if (fstat(f, &stb) > 0 && stb.st_blksize > 0)
			...
			blksize = BLKSIZE;
		buf = (unsigned char *)malloc(2*blksize);
		if (buf == NULL) {
