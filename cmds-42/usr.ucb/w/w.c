...
#define KERNEL_FEATURES		/* actually need sys/features */
#endif  KERNEL_FEATURES
#include <sys/features.h>	/* MACH or BERKELEY vm? */
#endif	NeXT
#if	MACH
#include <sys/table.h>
#endif	MACH
...
#if	MACH
		if (use_mach_factor) {
			table(TBL_MACHFACTOR, 0, &la, 1, sizeof(la));
			printf(",  Mach factor:");
		}
		else {
			table(TBL_LOADAVG, 0, &la, 1, sizeof(la));
			printf(",  load average:");
		}
		for (i = 0; i < (sizeof(la.tl_avenrun)/sizeof(la.tl_avenrun[0]));
							i++) {
			if (i > 0)
				printf(",");
			printf(" %.2f", (float)la.tl_avenrun[i] / (float)la.tl_lscale);
		}
#else	NeXT
		printf(",  load average:");
...
#if	NeXT
		if (fread(&utmp, sizeof(utmp), 1, ut) == 0) {
#else	NeXT
...
readpr()
{
	...
	for (i = 0; i < pcount; i++) {
		...
		if (ret != KERN_SUCCESS) {
#if	1
			p = psets[i];	// compatibility
#else	1
			mach_error("host_processor_sets", r);
			exit(0);
#endif	1
		}
	...
	task_t	task;
{
	...
	if (ret != KERN_SUCCESS)
		return;
	(void)table(TBL_PROCINFO, pid, (char *)&pi, 1, sizeof(pi));
	if (pi.pi_status == PI_EMPTY || pi.pi_status == PI_ZOMBIE
				|| pi.pi_pgrp == 0 || pi.pi_ttyd == -1)
		return;
	if (table(TBL_UAREA, pi.pi_pid, (char *)&up, 1, sizeof(struct user)) != 1)
			return;
	if (np >= nproc) {
		nproc *= 2;
		pr = (struct pr *)realloc(pr, nproc*sizeof (struct pr));
	}
	p = &pr[np];
	np++;
	p->w_pid = pi.pi_pid;
	...
	if (up.u_ttyp == NULL)
		return;
	p->w_igintr = (((int)up.u_signal[2]==1) +
		    2*((int)up.u_signal[2]>1) + 3*((int)up.u_signal[3]==1)) +
		    6*((int)up.u_signal[3]>1);
	p->w_time = up.u_ru.ru_utime.tv_sec + up.u_ru.ru_stime.tv_sec;
	p->w_ctime = up.u_cru.ru_utime.tv_sec + up.u_cru.ru_stime.tv_sec;
	p->w_tty = pi.pi_ttyd;
	p->w_uid = pi.pi_uid;
	up.u_comm[14] = 0;	/* Bug: This bombs next field. */
	strcpy(p->w_comm, up.u_comm);
	/*
	...
	*/
	p->w_args[0] = 0;
	strcatn(p->w_args, getargs(p), ARGWIDTH);
	if (p->w_args[0]==0 || p->w_args[0]=='-' && p->w_args[1]<=' ' ||
			p->w_args[0] == '?') {
		strcat(p->w_args, " (");
		strcat(p->w_args, p->w_comm);
		strcat(p->w_args, ")");
	}
}
#else	NeXT
