...
char **argv;
{
	...
	if (sysflag)
		supfname = salloc (sprintf (buf,FILESUPDEFAULT,DEFDIR));
	else {
...
TREE *getcollhost (state,nhostsp)
int *state,*nhostsp;
{
	...
	if (*state == 2)
		...
		(*state)++;
	return (getcollhost (state,nhostsp));
}
...
getcoll ()
{
	...
	nhosts = 0;
	for (;;) {
		if ((t = getcollhost (&state,&nhosts)) == NULL) {
			finishup (SCMEOF);
			notify ((char *)NULL);
			return;
		}
...
char *from;		/* 0 if reading from network */
{
	...
	/* try destination directory */
		path (to,dpart,fpart);
		sprintf (tname,"%s/sup.%d",dpart,thispid);
		tof = open (tname,(O_WRONLY|O_CREAT|O_TRUNC),0600);
		if (tof >= 0)  break;
	/* try sup directory */
		if (thisC->Cprefix)  chdir (thisC->Cbase);
		sprintf (tname,"sup/sup.%d",thispid);
		tof = open (tname,(O_WRONLY|O_CREAT|O_TRUNC),0600);
		if (tof >= 0) {
			if (thisC->Cprefix)  chdir (thisC->Cprefix);
			break;
	...
	/* try base directory */
		sprintf (tname,"sup.%d",thispid);
		tof = open (tname,(O_WRONLY|O_CREAT|O_TRUNC),0600);
		if (thisC->Cprefix)  chdir (thisC->Cprefix);
		if (tof >= 0)  break;
	/* try /usr/tmp */
		sprintf (tname,"/usr/tmp/sup.%d",thispid);
		tof = open (tname,(O_WRONLY|O_CREAT|O_TRUNC),0600);
		if (tof >= 0)  break;
	/* try /tmp */
		sprintf (tname,"/tmp/sup.%d",thispid);
		tof = open (tname,(O_WRONLY|O_CREAT|O_TRUNC),0600);
		if (tof >= 0)  break;
		istemp = FALSE;
