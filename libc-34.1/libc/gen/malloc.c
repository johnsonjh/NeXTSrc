...
struct malloc_static_t {
    ...
    int lasthitregion;       /* cache used in findregion */
    NXZone  *defaultzone;
    void    *freeme;
    mutex_t lock;
    char inited;
...
static void *_valloczone(size_t *size);
static region_t *findregion(void *ptr);
static region_t *addregion(char *start, int size, zone_t *zonep);
...
static void  keepzone(zone_t *zonep);
static void delregion(region_t *regionp);
static void delzone(zone_t *zonep);
static void joinzones();
...
static void newzonedata(zone_t *zonep, void **ptr, int size);
...
static void *nxzonemalloc(NXZone *zonep, size_t size);
...
static void *moremem(zone_t *zonep, size_t nsize);
...
struct mutex  stacklock;
    ...
    malloc_static.max_allocedregion = 3;
    ...
    malloc_static.regions = (region_t *)MALLOC(3*sizeof(region_t));
...
felem_t	  *fp;
	...
	if(malloc_static.regions[i].zonep == zonep) {
	    ...
	    headerp[-1].size = 3*sizeof(header_t);
	    nxzonefreenolock((NXZone *)parentzonep,headerp);
	      /* free end piece in parent zone */
	    headerp = (header_t *)((int)regionp->endadd - sizeof(header_t));
	    nxzonefreenolock((NXZone *)parentzonep,headerp);
	    if(i > 0) 
		malloc_static.regions[i-1].endadd = 
			    malloc_static.regions[i].endadd;
...
/*
 * Join areas in the region array which are continous and in the 
 * same malloc zone.
 */
static void joinzones()
{
...
LOCK {
    ...
    zonep->noshrink = 0;
    zonep->name = 0;
    datap = _valloczone(&startsize);
    newzonedata(zonep, (void **)&datap, startsize);
    checkregionsallocation();

#define STARTHUNK (sizeof(felem_t)*NUMFREES + sizeof(header_t))
	...
	datap = (header_t *) &freearray[NUMFREES];
	datap->size = startsize - STARTHUNK - sizeof(header_t);
...
region_t *newregionp;
	...
	if(i < (regionnum - 1) && malloc_static.regions[i+1].zonep == zonep &&
	    ...
	    *datap = *((header_t*)(malloc_static.regions[i-1].endadd - sizeof(header_t)));
	    ...
	    delregion(&malloc_static.regions[i+1]);
	    *ptr = datap;
	    return;
	}
	else {
...
#define DUMMYSIZE (2*sizeof(header_t))
      /*
	...
	zonep->freemaxn = NUMFREES;
	datap = (header_t *) &freearray[NUMFREES];
	datap->size = startsize - STARTHUNK - DUMMYSIZE - 2*sizeof(header_t);
	datap->lastguyfree = 0;
	datap->free = 0;
	...
	deadwoodp->size = 2*sizeof(header_t);
	zonep->canfree = 1;
	nxzonefreenolock((NXZone*)zonep, datap + 1);
	zonep->canfree = canfree;
	keepzone(zonep);
...
char	*data;
    ...
    *size = round_page(*size);
    data = 0;
#ifdef TESTING    
    if (MYvm_allocate(task_self(),(vm_address_t *) &data, *size, 1) != KERN_SUCCESS)
#else
    if (vm_allocate(task_self(),(vm_address_t *) &data, *size, 1) != KERN_SUCCESS)
#endif    
...
int	end;
	...
	regionp->endadd = start;
	    /* add bottom */
	addregion(start+size,end - ((int)start + size),regionp->zonep);
    }
    
    if(newzonep) 
	addregion(start,size,newzonep);
}
...
{
char *freelater;
    if((malloc_static.max_region+2) > malloc_static.max_allocedregion) {
...
header_t	*new,*old;
    ...
    if(size <= 0)
	size = 1;
    size = (size + sizeof(header_t) + 3) & ~3;  /* round up by 4 */
...
#define MINMEMORYSIZE  (sizeof(header_t) + sizeof(int))	
 ...
 */
static void *moremem(zone_t *zonep, size_t size)
{
header_t *datap,*deadwoodp;
void	*newptr;
int	nsize;
int	canfree;
    ...
    /* get more memory */
    nsize = size + 4*sizeof(header_t); /* dummy block + dead wood piece at end */
    if(nsize < zonep->granularity)
	nsize = zonep->granularity;
	
    if(zonep->type == EXTERNAL) {
	datap = (header_t *)_valloczone((size_t *)&nsize);
	if(!datap)
	    return(0);
	newzonedata(zonep,(void **) &datap, nsize);
    }
    else {
	if(!(datap = internal_zone_extend(zonep->parentzonep, zonep, nsize))) {
	    datap = nxzonemallocnolock((NXZone*)zonep->parentzonep, nsize);
	    nsize = malloc_size(datap); /* could have rounded up */
		/* Break up old region and insert internal */
		...
		*/
		datap->size = 2*sizeof(header_t);
		datap->free = 0;
		datap->lastguyfree = 0;
		datap += 2;
		nsize -= 2*sizeof(header_t);
			/* add dead wood at end */
		deadwoodp = (header_t *)((int)datap + nsize - 2*sizeof(header_t));
		deadwoodp->free = 0;
		deadwoodp->lastguyfree = 1;
		deadwoodp->size = 2*sizeof(header_t);
		nsize -= 2*sizeof(header_t);
...
header_t *deadwoodp,*datap;
	...
	if(malloc_static.regions[i].zonep == zonep) {
		...
		malloc_static.regions[i+1].beginadd = malloc_static.regions[i].endadd;
			/* add dead wood at end */
		deadwoodp = (header_t *)
			((int)malloc_static.regions[i].endadd - 2*sizeof(header_t));
		deadwoodp->free = 0;
		deadwoodp->lastguyfree = 1;
		deadwoodp->size = 2*sizeof(header_t);
			/* backup over last deadwood on return */
		datap = (header_t*)((int)add + oldsize - 2*sizeof(header_t));
		datap->free = 0;
...
region_t *regionp;
    ...
    regionp = findregion(ptr);
    if(regionp)
	return((NXZone*)regionp->zonep);
    else
	return(NX_NOZONE);
}
...
header_t *headerp;
	...
	if(regionp->zonep) {
		...
		if((int)ptr >= (int)headerp && 
		    ...
		    printf("start add 0x%x size %d free %d\n",headerp+1,
		    	headerp->size - sizeof(header_t),headerp->free);
		    printf("zone 0x%x ",regionp->zonep);
	    ...
	    printf("valloced -- start add 0x%x size %d free 0\n",
	    	regionp->beginadd,regionp->endadd - regionp->beginadd);
	    return;
	   }
	    		
	
    }
    printf("ptr not in heap\n");
}
...
header_t	*headerp,*next,*newp;
    ...
    if(size <= 0)
	size = 1;
    size += sizeof(header_t) + 3;
    size &= ~3;  /* round up by 4 */
...
#define GIVEBACK
    ...
    */
	...
	felem_t	*freearray;
	    ...
	    startpage = (void *)round_page((void *)headerp);
	    if(headerp == startpage) {
		regionp = findregion(headerp);
		...
		if(regionp > malloc_static.regions)
		    ...
		    lastregionp = 0;
		if(((int)regionp->beginadd == (int)headerp && 
		...
		!(lastregionp && lastregionp->zonep && 
		    ...
		    if(vm_deallocate(task_self(),(int)headerp, 
		...
		if(vm_deallocate(task_self(),(int)(headerp + 1), 
...
header_t *headerp;
    ...
    for(n = 1; n <= maxn; n++) {
	printf("n %d size 0x%x data %x \n",n,freearray[n].size,freearray[n].data);
	if((2*n) <= maxn)
	    if(freearray[2*n].size > freearray[n].size)
		printf("**Bad heap: left child to big 0x%x\n",freearray[2*n].size);
	if((2*n+1) <= maxn)
	    if(freearray[2*n+1].size > freearray[n].size)
		printf("**Bad heap: right child to big 0x%x\n",freearray[2*n+1].size);
	headerp = (header_t *)freearray[n].data;
	if(!headerp->free)
	    printf("**Bad free area not marked free\n");
	if(headerp->size != freearray[n].size)
	    printf("**Bad heap: free area size is 0x%x\n",headerp->size);
	i = ((int *)headerp)[headerp->size / sizeof(int) - 1];
	if(i != n)
	    printf("**Bad heap: back pointer to free heap is bad\n");
	}
...
char 	*type;
zone_t  *zonep;
    printf("number of regions used %d allocated %d\n",malloc_static.max_region,
	...
	if(zonep != 0) {
	    ...
	    type = "VALLOC";
	printf("beginadd %x endadd %x zone 0x%x type %s ",
		malloc_static.regions[i].beginadd,
		malloc_static.regions[i].endadd,zonep,type);
	if(zonep && zonep->name)
	    printf("%s\n",zonep->name);
	else
	    printf("\n");
    }
    printf("number of zones used %d allocated %d\n",malloc_static.max_zone,
    		malloc_static.max_allocedzone);
    for(i = 0; i < malloc_static.max_zone; i++) {
	printf("freen %d freemaxn %d \n",malloc_static.zones[i]->freen,
		malloc_static.zones[i]->freemaxn);
	printf("The free heap ---------------\n");
...
felem_t *lastfreearray;
	...
	if(zonep != 0) {
	    ...
	    type = "VALLOC";
	printf("beginadd %x endadd %x zonep 0x%x type %s ",
		malloc_static.regions[i].beginadd,
		malloc_static.regions[i].endadd, zonep, type);
	...
	if(zonep != 0) {
	    ...
	    headerp = (header_t *) malloc_static.regions[i].beginadd;
	    while((int)headerp < (int)malloc_static.regions[i].endadd) {
		printf("add 0x%x size 0x%x free %d lastguyfree %d\n",
		    headerp,headerp->size,headerp->free,
		    headerp->lastguyfree);
		if(headerp->size == 0) {
		    printf("***bad piece at %x size of zero\n",headerp);
		    return;
		}
		if(headerp->size == 4) {
		    printf("-------- extend point --------\n",headerp);
		...
		if(headerp->lastguyfree) {
		    lastspot = ((int *)headerp)[-1];
		    if(lastspot < 1 || lastspot > zonep->freen) {
			printf("back pointer number to the lastfree is bad\n");
			lastspot = 1;
		    }
		    lastfreearray = &zonep->freearray[lastspot];
		    if((int)(lastfreearray->data) != (int)lastheaderp) {
			printf("back pointer to last free is bad\n");
		    }
		    if(!lastheaderp->free) {
			printf("lastfree is not really free\n");
		    }
		}
		lastheaderp = headerp;		
		headerp = (header_t *)((int)headerp + headerp->size);
	    ...
	    if(lastheaderp->size > 8 && !((i < (malloc_static.max_region - 1) &&
	    	malloc_static.regions[i+1].zonep->type == INTERNAL))) {
		printf("last piece in region is bad\n");
	    }
...
{
void	*new;
    new = _valloczone(&size); 
    if(!new)
	return(0);
    addregion(new, size, 0);
    checkregionsallocation();
    return(new);
}
...
void	*new;
    ...
    new = NXZoneRealloc(malloc_static.defaultzone,oldptr,newsize);
    return(new);
}
...
region_t   *regionp;
	...
	if(regionp->beginadd == ptr) {
	    if(vm_deallocate(task_self(),(int)regionp->beginadd, 
		    (int)(regionp->endadd - regionp->beginadd)) != KERN_SUCCESS)
		MallocErrorFunc(MALLOC_ERROR_VM_DEALLOC);
	    delregion(regionp);
	}
 ...
 */
void NXNameZone(NXZone *z, char *name)
{
zone_t	*zonep = (zone_t *)z;
...
static void DefaultMallocError(int x) {
    static const char *MallocErrorMessages[] = {
	"vm_allocate failed\n",
...
/* 
 * Interface to allow other alloctor writers to register zones.
 */
void NXAddRegion(int start, int size,NXZone *zonep)
{
    addregion((char *)start, size, (zone_t *)zonep);
    checkregionsallocation();
}
...
int		size,regionsize,zonesize;
	...
	startcp = cp = (char *)_valloczone((size_t *)&size);
	if(!cp)
	    return(0);
...
/*
 * Fed an int returned prevously from malloc_freezedry() will restore malloc
 * state. 
 * Returns 
...
zone_t		*zonep;
    ...
    vm_deallocate(task_self(), (vm_address_t) startcp, (int)(cp - startcp));
}
    ...
    *
    * Do
    * cc -DMAIN -DTESTING -DMULTITEST -g malloc.c
    * a.out bignumber
    */
struct uf  {
  char *a;
...
int un,size,r,what,count,canfree,seed;
char *v;
NXZone *z1,*z2;
...
int   i;
    ...
    for(i = 0; i < n ; i++)
	if(ptr[i] != 1) {
	    printf("mem not zero ptr 0x%x n %d i %d\n",ptr,n,i);
	    abort();
	}
}
