...
#define SAME 0
#ifdef NeXT_MOD
#define SLCKTIME 5400	/* system/device timeout (LCK.. files) in seconds (90 mins) */
#else
...
#ifdef NeXT_MOD
#define LOCKPRE "/usr/spool/uucp/LCK/LCK."
#else
	...
	time_t atime;
{
	...
	if (pid < 0) {
		pid = getpid();
#ifdef NeXT_MOD
		sprintf(tempfile, "/usr/spool/uucp/LCK/LTMP.%d", pid);
#else
