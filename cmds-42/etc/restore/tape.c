...
#if	NeXT
	vprintf(stdout, "Verify %s and initialize maps\n",
			(ejectable ? "disk" : "tape"));
#else
...
#if	NeXT
	map = calloc((unsigned)1, (unsigned)howmany(VOLSIZE, NBBY));
	volmap = map;
#endif	NeXT	
	...
	while (newvol <= 0) {
		if (tapesread == 0) {
			...
			if (ejectable)  {
				...
				"You have not read data from any disks yet.\n",
				"Unless you know which disk volume your",
				" file(s) are on you should start\n",
				"with the first (1) volume and work",
				" towards towards the last.  You will\n",
				"be prompted for disks as they are needed.\n");
			}
			else
#endif	NeXT	
...
#if	NeXT
	if (ejectable) {
		fprintf(stderr, "Mounting disk volume %d\n", newvol);
	}
	else {
		fprintf(stderr, "Mount tape volume %d\n", newvol);
		fprintf(stderr, "then enter tape name (default: %s) ", 
			magtape);
		(void) fflush(stderr);
		(void) fgets(tbf, BUFSIZ, terminal);
		if (feof(terminal))
...
gethdr:
	...
	(void) gethead(&spcl);
	if ((curfile.action != USING) && (checktype(&spcl, TS_ADDR) != GOOD))
...
#ifdef	NeXT
	if (!gettingfile)
		if (setjmp(restart))		// second time
			if (checktype(&spcl, TS_ADDR) != GOOD) // continuations only
				return;
#else	NeXT
...
getmore:
	...
	if (i < 0) {
#if	NeXT	
		fprintf(stderr, "Tape/disk read error while ");
		switch (curfile.action) {
		default:
			fprintf(stderr, "trying to set up tape/disk\n");
#else
...
#endif
	if (i <= 0) {
		perror("Tape/disk read error");
		done(1);
	}
	if (i % TP_BSIZE != 0) {
		fprintf(stderr, "Tape/disk block size (%d) %s (%d)\n",
			i, "is not a multiple of dump block size", TP_BSIZE);
		done(1);
	}
	ntrec = i / TP_BSIZE;
	vprintf(stdout, "Tape/disk block size is %d\n", ntrec);
}
...
#if	NeXT
	if (ejectable)  {
		fprintf(stderr, "Ejecting disk volume.\n");
#ifdef RRESTORE
		rmteject(magtape);
#else
		(void) ioctl (mt, DKIOCEJECT, 0);
#endif
	}
#endif	NeXT	
#ifdef RRESTORE
