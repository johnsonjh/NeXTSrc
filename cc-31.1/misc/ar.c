...
#ifdef NeXT_MOD
	int xf, i;
	char tmparnam[MAXPATHLEN];
#endif NeXT_MOD

	if ((qf = open(arnam, 2)) < 0) {
#ifdef NeXT_MOD
		if ((xf = open(arnam, 0)) >= 0) {
			if (read(xf, mbuf, SARMAG) != SARMAG
			...
			lseek(xf, 0L, 0);
			strcpy(tmparnam, "TMP.");
			strcat(tmparnam, arnam);
			if ((qf = creat(tmparnam, 0666)) >= 0) {
				while((i = read(xf, buf, MAXBSIZE)) > 0)
					if (write(qf, buf, i) != i) {
				...
				i = sigblock(-1);
				if (unlink(arnam) >= 0
				   && link(tmparnam, arnam) >= 0) {
					sigsetmask(i);
					close(xf);
					lseek(qf, (long)SARMAG, 0);
					unlink(tmparnam);
					return;
				}
				sigsetmask(i);
...
match()
{
	...
	for(i=0; i<namc; i++) {
		...
		if(
#ifdef NeXT_MOD
		    strcmp(trim(namv[i], 0), file) == 0
#else !NeXT_MOD
		    strcmp(trim(namv[i]), file) == 0
#endif !NeXT_MOD
		) {
			file = namv[i];
			namv[i] = 0;
