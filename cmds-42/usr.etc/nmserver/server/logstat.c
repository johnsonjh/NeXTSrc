 ...
 * logstat.c
 *
 * $Source: /private/Net/p1/BSD/CMDS/cmds-14/usr.etc/nmserver/server/RCS/logstat.c,v $
 *
 */
#ifndef	lint
char logstat_rcsid[] = "$Header: logstat.c,v 1.4 89/06/13 15:44:57 osdev Exp $";
#endif not lint
/*
 ...
 */
#define	DEFAULT_CONFIG_FILE	"/etc/NETMSG_CONFIG"
 ...
 * Side effects:
 *
 * Writes both the old and the current logs into a file named "/tmp/NMLOG".
 * Resets the logs.
 *
 * Note:
 *	Locks the log_lock whilst manipulating the logs.
...
fd = open("/tmp/NMLOG", O_WRONLY|O_CREAT, 0644);
if (fd < 0) {
...
BEGIN("ls_mem_list")
	...
	*bucket_ptr = NULL;
	...
	kr = vm_allocate(task_self(),(vm_address_t *)class_ptr,class_max,TRUE);
	kr = vm_allocate(task_self(),(vm_address_t *)nam_ptr,nam_max,TRUE);
	kr = vm_allocate(task_self(),(vm_address_t *)bucket_ptr,bucket_max,TRUE);
