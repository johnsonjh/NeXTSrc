#ifndef lint
static char sccsid[] = 	"@(#)nfs_trace.c	1.1 88/03/07 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
	...
	nfs_fh *fh;
{
	...
	printf("fh=[%d, %d, %d]", ip[0], ip[1], ip[2]);
}
...
p_fattr(f)
	fattr *f;
{
	printf("type=%u, mode=%o, nlink=%u, uid=%u, gid=%u, size=%u,\
 blocksize=%u, rdev=%u, blocks=%u, fsid=%u, fileid=%u",
...
p_sattr(s)
	sattr *s;
{
	printf("mode=%o, uid=%u, gid=%u, size=%u", 
	...
	readlinkres *r;
{
	p_nfsstat(&r->status);
	if (r->status == NFS_OK) {
		printf(", data=%s", r->readlinkres_u.data);
	}
}
...
{
	p_fhandle(&r->file);
	printf(", offset=%u, count=%u", r->offset, r->count);
}
	...
	readres *r;
{
	...
	if (r->status == NFS_OK) {
		...
		printf(", len=%u, data=(data)", r->readres_u.reply.data.data_len);
	}
}
...
{
	p_fhandle(&w->file);
	printf(", offset=%u, len=%u, data=(data)", 
...
{
	p_fhandle(&args->dir);
	printf(", cookie=%d, count=%d", *(int *)args->cookie, args->count);
}
...
{
	while (p != NULL) {
		printf("(fileid=%u, name=%s, cookie=%u), ", 
	...
	readdirres *res;
{
	...
	if (res->status == NFS_OK) {
		...
		printf(", eof=%d", res->readdirres_u.reply.eof);
	}
}
...
trace_call(procnum, args)
	...
	char *args;
{
	printf("%s call(", procs[procnum].name);
...
trace_return(procnum, res)
	...
	char *res;
{
	printf("%s return(", procs[procnum].name);
