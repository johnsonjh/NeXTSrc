     ...
     tree init_list;
{
  if (TYPE_USES_VIRTUAL_BASECLASSES (type))
	...
	CLASSTYPE_MARKED6 (TREE_TYPE (vbases)) = 1;
      ...
      for (vbases = result; vbases; vbases = TREE_CHAIN (vbases))
	{
	  tree addr = TREE_OPERAND (TREE_VALUE (vbases), 0);
	  tree ref = build_indirect_ref (addr, 0);
	  tree init;

	  basetype = TREE_TYPE (TREE_TYPE (TREE_VALUE (vbases)));
	  /* Don't initialize twice.  */
	  if (CLASSTYPE_MARKED6 (basetype) == 0)
	    continue;

	  init = value_member (basetype, init_list);
	  if (init)
     ...
     int use_global_delete;
{
  ...
  if (TREE_CODE (type) == POINTER_TYPE)
  ...
  else
    {
	 ...
	 destructor.  If the destructor is visible, call it,
      ...
      addr = convert_force (build_pointer_type (type),
  ...
  if (TYPE_HAS_DESTRUCTOR (type))
	...
	exprstmt = build_tree_list (NULL_TREE, cond);
	  ...
	  && TYPE_NEEDS_DESTRUCTOR (basetype))
	{
	  expr = build_delete (basetype, ref, parent_auto_delete, flags, 0);
      ...
      for (i = 2; i <= n_baseclasses; i++)
	{
	  ...
	  expr = build_delete (TYPE_POINTER_TO (basetype),
			       convert_pointer_to (basetype, addr),
			       integer_zero_node, flags, 0);
