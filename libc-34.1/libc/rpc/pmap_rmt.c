...
static char sccsid[] = 	"@(#)pmap_rmt.c	1.5 88/07/27 4.0NFSSRC Copyr 1988 Sun Micro";
#endif
...
enum clnt_stat 
clnt_broadcast_exp(prog, vers, proc, xargs, argsp, xresults, resultsp, eachresult, inittime, waittime )
	...
	int             waittime;       /* how long to wait*/
{
	...
	if ((sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)) < 0) {
		(void) syslog(LOG_ERR, "Cannot create socket: %m");
		stat = RPC_CANTSEND;
		goto done_broad;
	}
	if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &on, sizeof (on)) < 0) {
		(void) syslog(LOG_ERR, "Cannot set socket option SO_BROADCAST: %m");
		stat = RPC_CANTSEND;
		goto done_broad;
	}
	FD_ZERO(&mask);
