...
pr_comment()
{
	...
	if ( /* ps.bl_line && */ (s_lab == e_lab) && (s_code == e_code)) {
	    ...
	    /* 12/19/86 wrp don't indent end of function brace comments */
	    if (*s_code == '}' && (e_code == s_code + 1))
	    	ps.com_col = target_col + 1;
	    if (ps.com_col < target_col)
	      /* 12/19/86 wrp fixed this to use var tabsize */
	    	ps.com_col = target_col + tabsize - ((target_col-1) % tabsize);
...
emptyspace:
	while (*buf_ptr == ' ' || *buf_ptr == '\t')
	    buf_ptr++;
	if (buf_ptr >= buf_end){	/* 12/19/86 wrp without this, null */
	    fill_buffer();		/* can get inserted */
	    goto emptyspace;
	}
    }
    ps.comment_delta = 0;
