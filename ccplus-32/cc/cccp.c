...
/* Nonzero when -MD or -MMD switch is used. */

#ifdef MACH_MAKE_DEPEND
int mdflag = 0;
#endif  /* MACH_MAKE_DEPEND */
...
#ifdef NeXT
    { &cplusplus_include_defaults[3], "/usr/include" },
    { 0, "/usr/local/include" }
#else  /* NeXT */
    { 0, "/usr/include" }
#endif  /* NeXT */
...
#ifdef MACH_MAKE_DEPEND
  /* This was taken care of up above. */
#else  /* MACH_MAKE_DEPEND */
  else if (print_deps)
    deps_stream = stdout;
#endif  /* MACH_MAKE_DEPEND */
...
get_filename:
  ...
  if (*fbeg == '/') {
    ...
    for (; stackp; stackp = stackp->next) {
      if (stackp->fname) {
#ifdef NeXT
	/* Skip over "" stack entries entirely.
	   This way setting GCC_INCLUDE_DIR to "" will
	   avoid trying to open non-existent files. */
	if (stackp->fname[0] == 0)
	  continue;
#endif  /* NeXT */
...
#ifdef NeXT_OBJC
    if (!importing)
      add_import (f, fname);	/* Make sure this #include'd file is on the "seen" list for #import. */
#endif  /* NeXT_OBJC */
    finclude (f, fname, op);
...
} *import_file_list[IMPORT_HASH_SIZE];
...
char *f;
{
  ...
  hashval = import_hash(f);

  /* Attempt to find file in list of already imported files */
  i = import_file_list[hashval];
  ...
  fd = open(f, O_RDONLY, 0666);
  if (fd < 0)
    return(-2);
  fstat(fd, &sb);
  for(h=0;h<IMPORT_HASH_SIZE;h++) {
    i = import_file_list[h];
    while(i) {
  ...
  i->name = (char *)xmalloc(strlen(f)+1);
  strcpy(i->name, f);
  i->inode = sb.st_ino;
  i->dev = sb.st_dev;
  i->next = import_file_list[hashval];
  import_file_list[hashval] = i;
