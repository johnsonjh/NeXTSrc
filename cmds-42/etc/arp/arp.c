...
file(name, reverse)
	char *name;
	int reverse;	/* reverse first two fields if set */
#else	NeXT
file(name)
	char *name;
#endif	NeXT
{
	...
	if (reverse) {
		args[1] = &arg[0][0];	/* ether addr */
		args[0] = &arg[1][0];	/* hostname   */
	} else {
		args[0] = &arg[0][0];	/* hostname   */
		args[1] = &arg[1][0];	/* ether addr */
	}
	args[2] = &arg[2][0];
	args[3] = &arg[3][0];
	args[4] = &arg[4][0];
...
#if	NeXT
	printf("       arp -a [/mach] [/dev/kmem]\n");
#else	NeXT
	printf("       arp -a [/vmunix] [/dev/kmem]\n");
#endif	NeXT
	printf("       arp -d hostname\n");
	printf("       arp -s hostname ether_addr [temp] [pub] [trail]\n");
	printf("       arp -f filename (file entries \"name eaddr ...\")\n");
	printf("       arp -e filename (file entries \"eaddr name ...\")\n");
}
