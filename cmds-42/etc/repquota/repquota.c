...
#ifndef lint
static char sccsid[] = "@(#)repquota.c	1.2 88/03/15 4.0NFSSRC"; /* from	5.4 (Berkeley) 3/5/86 */
#endif not lint
...
again:
	...
	if (aflag) {
		listp = listbuf;
		listcnt = 0;
		if ((fstab = setmntent(NULL, "r")) == NULL) {
			fprintf(stderr, "Can't open filesystem table\n");
			exit(8);
		}
		while (mntp = getmntent(fstab)) {
			/*
			 * Allow for NFS 3.2 vs NFS 4.0 mnttype compatibility.
			 * 	Morris Meyer  9/8/89
			 */ 
