 ...
 *	Changed usetenex variable to cmdedit.  Added ShellTypeAheadToTenex
 *	variables for support of CMU command-line editing.  Added extern
 *	declaration for havhash.
 *
 * 23-Jan-85  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Tell types.h to not create bool enum.
 *
 * 20-Oct-83  Duane Williams (dtw) at Carnegie-Mellon University
 *	Added declaration of usetenex variable.
 *
 **********************************************************************
 */
 ...
 */
#if	CMUCS
bool	cmdedit;		/* Use tenex routine for input */
#endif	CMUCS
bool	chkstop;		/* Warned of stopped jobs... allow exit */
bool	didfds;			/* Have setup i/o fd's for child */
volatile bool	doneinp;	/* EOF indicator after reset from readc */
...
volatile bool	setintr;	/* Set interrupts on/off -> Wait intr... */
...
#if	CMUCS
char	*ShellTypeAheadToTenex;	/* Read-ahead buffer for tenex () */
#endif	CMUCS
...
#if	NeXT
int	gnleft;				/* NCARGS is 40960 in <sys/param.h> */
#else
short	gnleft;
#endif	NeXT
...
#ifdef NeXT_MOD
char    *calloc();
#define calloc(i,j) ((alloctmp = calloc(i,j)) ? alloctmp : (char *)nomem(i*j))
