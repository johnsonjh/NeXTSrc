...
#define	REBOOT	"/usr/etc/reboot"
#define	HALT	"/usr/etc/halt"
#define MAXINTS 20
#define	HOURS	*3600
#define MINUTES	*60
#define SECONDS
...
char    nosyncflag[] = "-n";
#ifdef	NeXT_MOD
char	*power_down = NULL;
char	power_down_flag[] = "-p";
#endif	NeXT_MOD
	...
	char **argv;
{
...
#ifdef	NeXT_MOD
		printf("Usage: shutdown [ -krhfnp ] shutdowntime [ message ]\n");
#else	NeXT_MOD
	...
	for (;;) {
		...
		if (stogo <= 0) {
			...
			if (doreboot)
			    if (power_down)
				execle(REBOOT, "reboot", "-l", power_down,
				       nosync, 0, 0);
			    else
				execle(REBOOT, "reboot", "-l", nosync, 0, 0);
			if (halt)
			    if (power_down)
				execle(HALT, "halt", "-l", power_down,
				       nosync, 0, 0);
			    else
