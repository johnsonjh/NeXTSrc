...
char *first_global_object_name = 0;
     ...
     tree decl;
{
  ...
  if (DECL_CONTEXT (decl) != 0 && DECL_INITIAL (decl) != 0
      && DECL_RTL (decl) == 0)
    {
...
/* Output assembler code for the constant pool of a function and associated
   ...
   For the constant pool, we use the current constant pool data.  */
     ...
     int at_end;
{
  ...
  /* ANSI specifies that a tentative definition which is not merged with
     ...
     There is now s awitch (-nocommon) to give the ANSI behaviour.
     -pedantic will also do this.  */

  if (! (flag_no_common || pedantic) && 
  ...
  align = DATA_ALIGNMENT (decl, DECL_ALIGN (decl));
#else /* no DATA_ALIGNMENT */
     ...
     enum machine_mode mode;
{
  ...
  if (REAL_VALUES_EQUAL (dconst0, d))
    ...
    return (mode == DFmode ? dconst1_rtx : fconst1_rtx);
...
force_const_double_mem (r)
     rtx r;
{
  if (GET_CODE (CONST_DOUBLE_MEM (r)) != MEM)
    force_const_mem (GET_MODE (r), r);
...
#ifdef DATA_ALIGNMENT
  align = DATA_ALIGNMENT (exp, TYPE_ALIGN (TREE_TYPE (exp)));
#else
     ...
     register int size;
{
  ...
  switch (code)
      ...
      if (TYPE_MODE (TREE_TYPE (exp)) == DImode)
	{
      ...
      if (size == GET_MODE_SIZE (QImode))
	{
	  ASM_OUTPUT_CHAR (asm_out_file, x);
	  size -= GET_MODE_SIZE (QImode);
	}
      else if (size == GET_MODE_SIZE (HImode))
	{
	  ASM_OUTPUT_SHORT (asm_out_file, x);
	  size -= GET_MODE_SIZE (HImode);
	}
      else if (size == GET_MODE_SIZE (SImode))
	{
	  ASM_OUTPUT_INT (asm_out_file, x);
	  size -= GET_MODE_SIZE (SImode);
	}
#ifdef ASM_OUTPUT_DOUBLE_INT
      else if (size == GET_MODE_SIZE (DImode))
	{
	  ASM_OUTPUT_DOUBLE_INT (asm_out_file, x);
	  size -= GET_MODE_SIZE (DImode);
	}
