 ...
 * HISTORY
 * $Log:	tcp.c,v $
 * Revision 1.15.1.1  89/09/16  21:21:29  dpj
 * 	Moved increment of cp->count in tcp_sendrequest() to make it
 * 	more obvious that the connection records are correctly managed
 * 	(not a bug). Change suggested by avie@NeXT.
 * 	Fixed to not call SBUF_APPEND() for an empty segment.Change
 * 	suggested by avie@NeXT.
 * 	Added comments for a bug with TCP_CLOSING->TCP_CONNECTED
 * 	transitions. The bug never triggers with the way the module
 * 	is currently operating, and I'd rather not fix it
 * 	unless needed (see XXX BUG).
 * 
 ...
 */
#ifndef	lint
char tcp_rcsid[] = "$Header: tcp.c,v 1.15.1.1 89/09/16 21:21:29 dpj Exp $";
#endif not lint
/*
