...
struct section_map *section_map)
{
	 ...
	 */
	if(s->size % 4 != 0){
	    error_with_cur_obj("4 byte literal section (%0.16s,%0.16s) size is "
...
char *indent)
{
	...
	if(data == NULL)
	    return;
	print("%s   literal4_blocks 0x%x\n", indent, data->literal4_blocks);
	for(p = &(data->literal4_blocks); *p ; p = &(literal4_block->next)){
	    literal4_block = *p;
	    print("%s\tused %d\n", indent, literal4_block->used);
	    print("%s\tnext 0x%x\n", indent, literal4_block->next);
	    print("%s\tliteral4s\n", indent);
	    for(i = 0; i < literal4_block->used; i++){
		print("%s\t    0x%08x\n", indent,
		      literal4_block->literal4s[i].long0);
	    }
	}
}
...
struct merged_section *ms)
{
	if(data == NULL)
	    return;
	print("literal4 section (%0.16s,%0.16s) contains:\n",
