     ...
     tree type, init, *tail;
{
  ...
  if (element && (TREE_TYPE (element) == type
	...
	element = DECL_INITIAL (element);
      else if (TREE_READONLY (element) && TREE_CODE (element) == VAR_DECL)
...
#ifdef NeXT	/* This doesn't work for big-endian machines! --mself */
	tem1 = build_int_2 (TREE_INT_CST_LOW (r1)|(TREE_INT_CST_LOW (r0)<<(bitpos1-bitpos0)), 0);
#else  /* NeXT */
