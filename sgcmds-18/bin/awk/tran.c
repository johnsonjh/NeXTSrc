...
{
	char		s[100];
	static	char	*isfloat;	/* a float style format? */
	static	char	*isint;		/* an integer style format? */
	...
	if ((vp->tval & STR) == 0) {
		if (!(vp->tval&FLD))
			strfree(vp->sval);
		if (isfloat != *OFMT && isint != *OFMT) {    /* not set yet, or reset */
			char	*cp;
			
			for (cp = *OFMT; *cp; ++cp)
				;
			if (cp != *OFMT)
				--cp;			/* don't back over null */
			if (strchr("gfeGE", *cp))	/* ANSI float/double printf specs */
				isfloat = *OFMT;
			else
				isint = *OFMT;
		}
		if (isint == *OFMT)		/* cast to long format */
			(void)sprintf(s, *OFMT, (long)vp->fval);
		else
