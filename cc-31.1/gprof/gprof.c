...
#ifndef lint
static char sccsid[] = "@(#)gprof.c	5.1 (Berkeley) 6/4/85";
#endif not lint
...
getnfile()
{
    ...
    if (*(int *)&xbuf == MH_MAGIC) {
      mach_o = 1;
      execinfo = read_mach_o(nfd);
    } else if (N_BADMAG(xbuf)) {
    ...
    FILE	*nfile;
{
  ...
  if (mach_o) {
    ...
    /* First, initialize objective-C data structures. */
    if (!ignore_objc) {
      ml = mlist(fileno(nfile), 0, info->mach_header, info->load_commands,
		 &mlist_cnt);
      if(ml != NULL)
	use_objc = TRUE;
    }
    /* Mimic algorithm below... First pass */
    n_files = 0;
    while(info) {
      struct nlist *n;
      int i;
      ...
      strtab = info->strings;
      n = (struct nlist *)info->syms;
      for(i=0;i<info->nsyms;i++,n++) {
	if (funcsymbol(n))
	  nname++;
	if (n->n_type == N_SO) n_files++;
      }
      /* Next library */
      info = info->next;
    }
    ...
    nl = (nltype *) calloc( nname+1 , sizeof(nltype) );
    if (nl == 0) {
	fprintf(stderr, "%s: No room for %d bytes of symbol table\n",
		whoami, (nname+1) * sizeof(nltype) );
	done();
    }
	...
	if (Sflag && (n->n_type == N_SO)) {
	  if (oddeven) {
	    files[n_files++].lastpc = n->n_value;
	    oddeven = 0;
	  } else {
	    char *s = strtab + n->n_un.n_strx;
	    int len = strlen(s);
	    if (len > 0) s[len-1] = 'o';
	    files[n_files].name = files[n_files].what_name = s;
	    files[n_files].firstpc = n->n_value; 
	    oddeven = 1;
	  }
	}
	...
	if (funcsymbol(n)) {
	  ...
	  npe->value = n->n_value;
	  if (use_objc) {
	    mp = bsearch(&(n->n_value), ml, mlist_cnt, sizeof(struct mlist),
			 (int (*)(const void *, const void *))find);
	    if(mp != NULL)
	      npe->name = mp->m_name;
	    else
	      npe->name = strtab+n->n_un.n_strx;
	  }
	  else
	    npe->name = strtab+n->n_un.n_strx;
	  npe++;
...
#	endif DEBUG
  ...
  npe->value = npe->svalue = -1;
}

/*
 * Routine for bsearch to find methods
 */
static
long
find(
...
#if	NeXT_MOD
    if (!childp) {
	    fprintf(stderr, "bad arc from %d to %d, count %d (ignored).\n",
...
#   ifdef DEBUG
	...
	if ( debug & TALLYDEBUG ) {
	  if (parentp)
	    ...
	    printf( "[tally] arc from %d to %s traversed %d times\n" ,
		    rawp -> raw_frompc , childp -> name , rawp -> raw_count );
	}
#else
	if ( debug & TALLYDEBUG ) {
	    printf( "[tally] arc from %s to %s traversed %d times\n" ,
...
#if NeXT_MOD
	    if (parentp) addarc( parentp , childp , rawp -> raw_count );
#else
