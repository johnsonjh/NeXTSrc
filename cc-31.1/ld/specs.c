...
struct segment_spec *segment_specs = NULL;
long nsegment_specs = 0;
...
process_section_specs(void)
{
	...
	for(i = 0; i < nsegment_specs; i++){
		...
		if(strncmp(segment_specs[i].segname, segment_specs[j].segname,
			   sizeof(s.segname)) == 0)
		    error("segment names: %s and %s not unique to %d "
	...
	for(i = 0; i < nsegment_specs; i++){
		 ...
		 */
		if(sect_spec->contents_filename != NULL)
		    error("section (%0.16s,%0.16s) exist in the loaded object "
			...
			if(ms->s.align > sect_spec->align)
			    warning("specified alignment (0x%x) for section (%s"
				    ...
				    1 << sect_spec->align,
				    segment_specs[i].segname,
				    sect_spec->sectname, 1 << ms->s.align);
			else
			    ms->s.align = sect_spec->align;
		    }
...
#ifdef remove
			if(ms->s.flags != 0){
			    error("section (%0.16s,%0.16s) in the loaded "
				  "object files not a regular section and "
				  "can't have a -sectorder file: %s",
				  ms->s.segname, ms->s.sectname,
				  sect_spec->order_filename);
			}
			else{
	...
	for(i = 0; i < nsegment_specs; i++){
		...
		if(segment_specs[i].addr_specified){
			...
			error("address: 0x%x specified for segment: %s not a "
			      ...
			      segment_specs[i].addr, segment_specs[i].segname,
			      segalign);
		    }
		    else{
