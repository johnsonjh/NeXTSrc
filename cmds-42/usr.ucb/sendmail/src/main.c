...
#else
	unsigned long edata_value, end_value;
#endif OLD_LINKER
	extern char *sbrk();
	extern char Version[];
...
#else
	edata_value = get_edata();
	end_value = get_end();
	fhdr.frzinfo.frzedata = (char *) edata_value;
	fhdr.frzinfo.frzend = (char *) end_value;
#endif OLD_LINKER
...
#else
	    write(f, (char *) edata_value,
		  (int) (fhdr.frzinfo.frzbrk - edata_value)) !=
		  (int) (fhdr.frzinfo.frzbrk - edata_value))
#endif OLD_LINKER
	{
		syserr("Cannot freeze");
	...
	char *freezefile;
{
	...
	if (read(f, (char *) &fhdr, sizeof fhdr) < sizeof fhdr ||
	    ...
	    fhdr.frzinfo.frzend != &end ||
#else
	    fhdr.frzinfo.frzedata != (char *) get_edata() ||
	    fhdr.frzinfo.frzend != (char *) get_end() ||
#endif OLD_LINKER
	...
	if (read(f, (char *) &edata, (int) (fhdr.frzinfo.frzbrk - &edata)) !=
					(int) (fhdr.frzinfo.frzbrk - &edata))
#else
	if (read(f, (char *) get_edata(),
		(int) (fhdr.frzinfo.frzbrk - (char *)get_edata)) !=
		(int) (fhdr.frzinfo.frzbrk - (char *)get_edata))
#endif OLD_LINKER
	{
