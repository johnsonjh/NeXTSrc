 ...
 *	Adapted for 4.2 CMUCS environment.
 *
 * 10-Oct-83  Doug Philips (dwp) at Carnegie-Mellon University
 *	Added 'prpushlex' to push back a command entry, using the
 *	ShellTypeAheadToTenex variable.  prpushlex uses malloc.
 *
 * 02-Oct-83  Duane Williams (dtw) at Carnegie-Mellon University
 *	Changed test for tenex from intty to cmdedit.
 *
 **********************************************************************
 */
...
#if	NeXT
			while (c = getC(DOEXCL), letter(c) || digit(c)) {
				if (np < &name[sizeof name / 2])
					*np++ = c;
			}
#else		
