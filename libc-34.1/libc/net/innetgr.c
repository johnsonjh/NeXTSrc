...
#if	NeXT
static int lookup(char *thisdomain, char *map, char *group,
		  char *name, char *domain, int *res);
static doit(struct innetgrdata *d, char *group);
static int makekey(char *key, char *name, char *domain);
#endif	NeXT

_old_innetgr(group, machine, name, domain)
