	...
	char *cp;
{
	...
	return (cp < end);
#else
	return 1;	/* always assume might be writable */
#endif
}
...
#if	NeXT_MOD
		    /*
		     * Make sure we don't blow our real standard I/O descs
		     * away if we're sourcing. [gk 15Aug89]
		     */
		    f != FSHIN && f != FSHOUT && f != FSHDIAG && f!= FOLDSTD &&
#endif	NeXT_MOD
		    f != FSHTTY)
			(void) close(f);
}
