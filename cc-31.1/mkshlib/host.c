...
static void make_exthash(void);
static void scan_objects(void);
static void make_references(void);

static void build_archive(void);
static int ranlib_qsort(const struct ranlib *ran1, const struct ranlib *ran2);
static void build_lib_object(void);
static void build_ref_object(void);
static void build_host_objects(void);
static void init_symbol(struct object *op, char *name,
			struct relocation_info *reloc,
			long *value, long *defined, long symbolnum,
			long address);
static void write_archive(void);
static void write_lib_obj(char *buffer);
static void write_ref_obj(char *buffer);
static void write_object(struct object *op, char *buffer);
...
make_exthash(void)
{
    int fd, string_size, hash_key, i, nsyms;
	...
	/*
	 * Open the target shared library and check to see if is is a shared
	 * library and has a symbol table.
	 */
	if((fd = open(target_filename, O_RDONLY)) == -1)
	    perror_fatal("can't open target shared library: %s",
			 target_filename);
	if(read(fd, &target_mh, sizeof(struct mach_header)) !=
	   sizeof(struct mach_header))
	    perror_fatal("can't read mach header of target shared library: "
			 "%s", target_filename);
	if(target_mh.magic != MH_MAGIC)
	    fatal("bad magic number for target shared library: %s",
		  target_filename);
	if(target_mh.filetype != MH_FVMLIB)
	    fatal("bad file type (not MH_FVMLIB) for target shared "
		  "library: %s", target_filename);

	load_commands =(struct load_command *)xmalloc(target_mh.sizeofcmds);
	if(read(fd, load_commands, target_mh.sizeofcmds) !=
	   target_mh.sizeofcmds)
	    perror_fatal("can't read load commands of target shared "
			 "library: %s", target_filename);
	...
	for(i = 0; i < target_mh.ncmds; i++){
		...
		if(st != (struct symtab_command *)0)
		    fatal("more than one symtab command in target shared "
			  "library: %s", target_filename);
	...
	if(st == (struct symtab_command *)0)
	    fatal("no symtab command in target shared library: %s",
		  target_filename);
	if(st->nsyms == 0)
	    fatal("no symbol table in target shared library: %s",
		  target_filename);

	/* read the symbol table */
	symbols = (struct nlist *)xmalloc(st->nsyms * sizeof(struct nlist));
	lseek(fd, st->symoff, L_SET);
	if(read(fd, symbols, st->nsyms * sizeof(struct nlist)) !=
	   st->nsyms * sizeof(struct nlist))
	    perror_fatal("can't read symbol table of target shared "
			 "library: %s", target_filename);
	nsyms = st->nsyms;

	/* read the string table */
	strings = (char *)xmalloc(st->strsize + 1);
	strings[st->strsize] = '\0';
	lseek(fd, st->stroff, L_SET);
	if(read(fd, strings, st->strsize) != st->strsize)
	    perror_fatal("can't read string table of target shared "
			 "library: %s", target_filename);
...
scan_objects(void)
{
    long i, j, k, l, fd, nsyms, string_size, hash_key;
	...
	for(i = 0; i < nobject_list; i++){
	     ...
	     */
	    if((fd = open(object_list[i]->name, O_RDONLY)) == -1)
		perror_fatal("can't open object file: %s",
			     object_list[i]->name);
		lseek(fd, 0, L_SET);

	    if(read(fd, &mh, sizeof(struct mach_header)) !=
	       sizeof(struct mach_header))
		perror_fatal("can't read mach header of object file: %s",
			     object_list[i]->name);
	    if(mh.magic != MH_MAGIC)
		fatal("bad magic number for object file: %s",
		      object_list[i]->name);
	    if(mh.filetype != MH_OBJECT)
		fatal("bad file type (not MH_OBJECT) for object file: %s",
		      object_list[i]->name);

	    load_commands = (struct load_command *)xmalloc(mh.sizeofcmds);
	    if(read(fd, load_commands, mh.sizeofcmds) != mh.sizeofcmds)
		perror_fatal("can't read load commands of object file: %s",
			     object_list[i]->name);
		...
		if(lc->cmd == LC_SYMTAB){
		    if(st != NULL)
			fatal("more than one symtab command in object "
			      "file: %s", object_list[i]->name);
		    st = (struct symtab_command *)lc;
		}
		lc = (struct load_command *)((char *)lc + lc->cmdsize);
	    }
	    if(st == NULL)
		fatal("no symtab command in object file: %s",
		      object_list[i]->name);
	    ...
	    symbols = (struct nlist *)
		      xmalloc(st->nsyms * sizeof(struct nlist));
	    lseek(fd, st->symoff, L_SET);
	    if(read(fd, symbols, st->nsyms * sizeof(struct nlist)) !=
	       st->nsyms * sizeof(struct nlist))
		perror_fatal("can't read symbol table of object file: %s",
			     object_list[i]->name);
	    nsyms = st->nsyms;
	    ...
	    lseek(fd, st->stroff, L_SET);
	    if(read(fd, strings, st->strsize) != st->strsize)
		perror_fatal("can't string table of object file: %s",
			     object_list[i]->name);
	    string_size = st->strsize;
	    free(load_commands);
/*
...
*/
		...
		if(symbols[j].n_un.n_strx < 0 ||
		   symbols[j].n_un.n_strx > string_size)
		    fatal("bad string index for symbol %d in object file: %s ",
			  j, object_list[i]->name);
		...
		if(extp->alias != (struct ext *)0){
			...
			if(extp->op != (struct object *)0)
			    fatal("symbol: %s defined in both: %s and %s", p,
				  object_list[i]->name, extp->op->name);
...
*/
		...
		(struct nlist *)xmalloc(object_list[i]->nsymbols *
					sizeof(struct nlist));
	    object_list[i]->strings =
		(char *)xmalloc(object_list[i]->string_size);
	    object_list[i]->pu_symbols =
		(struct nlist *)xmalloc(object_list[i]->pu_nsymbols *
					sizeof(struct nlist));
	    object_list[i]->pu_strings =
		(char *)xmalloc(object_list[i]->pu_string_size);
	    k = 0;
	...
	for(i = 0; i < nobject_list; i++){
		...
		if((object_list[i]->symbols[j].n_type & N_TYPE) == N_UNDF){
			...
			if(refp == (struct ref *)0){
			    refp = (struct ref *)xmalloc(sizeof(struct ref));
...
build_archive(void)
{
	 ...
	 */
	toc_object.ranlibs = (struct ranlib *)xmalloc(toc_object.nranlibs *
						      sizeof(struct ranlib));
	toc_object.ranlib_strings =
		(char *)xmalloc(round(toc_object.ranlib_string_size,
				      sizeof(long)));
	...
	if(ran_index != toc_object.nranlibs)
	    fatal("internal error: ran_index (%d) not equal to "
		  "toc_object.nranlibs (%d)", ran_index, toc_object.nranlibs);
	if(ran_strx != toc_object.ranlib_string_size)
	    fatal("internal error: ran_strx (%d) not equal to "
		  "toc_object.ranlib_string_size (%d)", ran_strx,
		  toc_object.ranlib_string_size);
...
build_host_objects(void)
{
	...
	for(i = 0; i < nobject_list; i++){
	     ...
	     */
	    object_list[i]->symbols =
		(struct nlist *)xrealloc(object_list[i]->symbols,
				    (object_list[i]->nsymbols + new_symbols) *
				    sizeof(struct nlist));
	    object_list[i]->strings =
		(char *)xrealloc(object_list[i]->strings,
...
write_archive(void)
{
	...
	if((fd = open(host_filename, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1)
	    perror_fatal("can't create file: %s", host_filename);
	if((r = vm_allocate(task_self(), (vm_address_t *)&buffer,
	 ...
	 */
	if(fstat(fd, &stat) == -1)
	    perror_fatal("can't stat host library file: %s", host_filename);
	mtime = time(0);
	...
	sprintf(buffer + offset, "%-*.*s%-*ld%-*u%-*u%-*o%-*ld%-*s",
		...
		sizeof(ar_hdr.ar_name),
		    SYMDEF_SORTED,
		sizeof(ar_hdr.ar_date),
		    symdef_mtime,
		sizeof(ar_hdr.ar_uid),
		    uid,
		sizeof(ar_hdr.ar_gid),
		    gid,
		sizeof(ar_hdr.ar_mode),
		    mode,
	...
	*((long *)(buffer + offset)) = toc_object.ranlib_string_size;
	offset += sizeof(long);
	...
	if(write(fd, buffer, host_lib_size) != host_lib_size)
	    perror_fatal("can't write host library file: %s", host_filename);
#ifdef OS_BUG
	fsync(fd);
#endif
	if(close(fd) == -1)
	    perror_fatal("can't close file: %s", host_filename);
	if((r = vm_deallocate(task_self(), (vm_address_t)buffer,
