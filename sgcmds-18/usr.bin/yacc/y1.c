...
others(){ /* put out other arrays, copy the parsers */
    ...
    /* tell downstream tools (cc, gdb,) where this source is coming from */
	...
	while( (c=getc(finput) ) != EOF ){
		if( c == '$' ){
			if( (c=getc(finput)) != 'A' ) putc( '$', ftable );
				...
				ZAPFILE(ACTNAME);
				putc( '\n', ftable );
			/* resync PARSER linenumbers */
				fprintf( ftable, "# line %d \"%s\"\n",
						parser_linenum, PARSER );
				c = getc(finput);
				}
			}
		if( c == '\n' )		/* count parser linenumbers */
			parser_linenum++;
		putc( c, ftable );
		}
