...
#   endif CBREAK
						|VTDELAY|ALLDELAY);
		mode.sg_flags |= XTABS|ECHO|CRMOD;
		if (mode.sg_flags & (EVENP|ODDP))
			mode.sg_flags |= ANYP;
...
# ifndef USG
		...
		if (tgetflag("UC") || (command[0] & 0140) == 0100)
			...
			mode.sg_flags &= ~LCASE;
		old_parity = mode.sg_flags & (EVENP | ODDP);
...
char	def;
{
	...
	if (o == n)
		...
		prs(" set to ");
	bufp = buf;
	if (tgetstr("kb", &bufp) != NULL && n == buf[0] && buf[1] == NULL)
		prs("Backspace\n");
	else if (n == 0177)
		prs("Delete\n");
	else
