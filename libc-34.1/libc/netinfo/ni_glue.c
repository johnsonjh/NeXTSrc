...
typedef struct ni_private {
	...
	struct in_addr *addrs;	/* addresses of servers */
	    ...
	    )
{
	...
	if (sock < 0) {
		...
		if ((ifreq.ifr_flags & IFF_UP) && 
			...
			close(sock);
			return (((struct sockaddr_in *)&ifreq.ifr_addr
				 )->sin_addr.s_addr);
		}
	}
	close(sock);
...
/*
 * Returns a client handle to the netinfo server, if its running
 */
static int
connectlocal(
	     ni_private *ni
...
/*
 * Callback routine for manycaster
       ...
       )
{
	...
	for (;;) {
		...
		if (!printed) {
			debug("manycast timeout, sleeping");
			printed++;
		}
		sleep(sleeptime);
       ...
       )
{
	...
	for (;;) {
		...
		if (needwrite || ni->abort) {
			...
			debug("netinfo failure, aborting");
			return (NULL);
		}
		if (!printed) {
			if (ni->tc != NULL) {
				if (!(sleeptime == 0 &&
					 ...
					 * poor user!
					 */
					debug(clnt_sperror(ni->tc, 
							   "netinfo sleeping"));
					printed++;
				}
			} else {
				debug("netinfo failure, sleeping");
				printed++;
			}
		}
		if (sleeptime > 0) {
	...
	)
{
	...
	addr.s_addr = inet_addr(nl.ninl_val[0]);
	ni_namelist_free(&nl);
	if (target_ni->naddrs == 0) {
		target_ni->addrs = (struct in_addr *
				    )malloc(sizeof(struct in_addr));
		target_ni->tags = (ni_name *)malloc(sizeof(ni_name));
		target_ni->naddrs = 1;
	} else {
		target_ni->naddrs++;
		target_ni->addrs = (struct in_addr *
				    )realloc(target_ni->addrs,
					     (sizeof(struct in_addr) *
					      target_ni->naddrs));
		target_ni->tags = (ni_name *)realloc(target_ni->tags,
						     (sizeof(ni_name) *
						      target_ni->naddrs));
	}
	target_ni->addrs[target_ni->naddrs - 1] = addr;
	target_ni->tags[target_ni->naddrs - 1] = ni_name_dup(tag);
	return (1);
}
...
getparent(ni_private *oldni, ni_private **newni)
{
	...
	while (!found) {
		...
		*ni = *oldni;
		...
		ni->naddrs = 1;
		ni->addrs = (struct in_addr *)malloc(sizeof(struct in_addr));
		ni->addrs[0].s_addr = resp->ni_rparent_res_u.binding.addr;
		ni->tags = (ni_name *)malloc(sizeof(ni_name));
