 ...
 */
#if	NeXT
#define	TBL	"tbl"			/* for tbl processing */
#endif	NeXT
	...
	char *argv[];
{
	...
	if ((mp = getenv("TROFF")) != NULL)
		troffer = mp;
	if ((mp = getenv("MANPAGER")) != NULL)
		pager = mp;
...
#if	NeXT
		if (argc <= 1) {
			fprintf(stderr, "Usage: apropos keyword ...\n");
			exit(1);
		}
#endif	NeXT
...
#if	NeXT
		if (argc <= 1) {
			fprintf(stderr, "Usage: whatis command ...\n");
			exit(1);
		}
#endif	NeXT
	...
	char *pp, *wp;
{
	...
	chdir(pp);
#if	NeXT
	sprintf(cmdbuf, "%s %s | %s -man", TBL, wp, troffer);
#else	NeXT
	sprintf(cmdbuf, "%s -man", TROFF, wp);
#endif	NeXT
	(void) system(cmdbuf);
}
