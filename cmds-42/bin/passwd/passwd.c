...
#ifndef lint
static char sccsid[] = 	"@(#)passwd.c	1.6 88/08/06 4.0NFSSRC SMI"; /* from UCB 4.24 5/28/86 */
#endif not lint
...
#define	PWDIR		"/etc/"
#define PWPATH(x)	PWDIR #x

char	temp[] = PWPATH(ptmp);
char	passwd[] = PWPATH(passwd);
#ifdef SECURE_RPC
	...
	char *argv[];
{
	...
	if (argc < 1) {
		...
		(void) printf("Changing %s for %s on %s.\n",
	...
	if (dochfn)
		...
		cp = getnewpasswd(pwd, u, &old_pass);
#if	NeXT
	/*
	 * If NetInfo is running, then change the user's NetInfo password
	 */
	if (ni_new(NULL, ".") != NULL) {
		if (dochfn || dochsh) {
			fprintf(stderr, 
				"Cannot change finger information or "
				"shell with NetInfo (yet).\n");
			exit(1);
		}
		if (putpwpasswd(uname, old_pass, cp) != 1) {
	...
	char **old_pass;
{
	...
	if (pwd->pw_passwd[0] && u != 0) {
		...
		/*
		 * Save unencrypted password to pass to putpwpasswd()
		 */
		*old_pass = strcpy(malloc(strlen(pwbuf) + 1), pwbuf);
	}
tryagain:
...
#else	NeXT
	/*
	 * Why would you not allow someone to change their default
	 * shell back to /bin/sh?  Am changing this so it is possible
	 * to go from /bin/sh to /bin/csh.
	 * 		MAM  March 28, 1989
	 */
