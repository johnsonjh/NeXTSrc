     ...
     int strict, flags;
{
  ...
  /* @@ Perhaps this should try to go through a constructor first
     ...
     @@ between references to builtin types.  I am beginning
     ...
     tree val;
{
 ...
 if (TREE_CODE (type) == REFERENCE_TYPE)
      ...
      if (TREE_READONLY (type))
	{
	  ...
	  if (TREE_CODE (val) == VAR_DECL && TREE_READONLY (val)
	      && DECL_INITIAL (val))
     ...
     tree type, expr;
{
  ...
  if (IS_AGGR_TYPE_CODE (code))
      ...
      if (IS_AGGR_TYPE (dtype))
	{
	  ...
	  if (TYPE_HAS_CONSTRUCTOR (type))
	    {
	      tree rval = build_method_call (NULL_TREE, DECL_NAME (TYPE_NAME (type)), build_tree_list (NULL_TREE, e), CLASSTYPE_AS_LIST (type), 0);
     ...
     int for_sure;
{
  ...
  if (type_default != type)
  ...
  else if (type == ptr_type_node)
    {
	 ...
	 with which void* is compatible.  */
      basetype = save_basetype;

      while (TYPE_HAS_CONVERSION (basetype))
	{
	  if (CLASSTYPE_PTR_CONVERSION (basetype) != 0)
	    {
	      if (CLASSTYPE_PTR_CONVERSION (basetype) == error_mark_node)
		return error_mark_node;
	      typename = DECL_ORIGINAL_NAME (CLASSTYPE_PTR_CONVERSION (basetype));
	      return build_type_conversion_1 (xtype, basetype, expr, typename, for_sure);
	    }
	  if (CLASSTYPE_N_BASECLASSES (basetype))
	    basetype = CLASSTYPE_BASECLASS (basetype, 1);
	   ...
	   && TYPE_MAIN_VARIANT (TREE_TYPE (type)) == void_type_node)
    {
	 ...
	 with which const void* is compatible.  */
      ...
      while (TYPE_HAS_CONVERSION (basetype))
	{
	  if (CLASSTYPE_CONSTPTR_CONVERSION (basetype) != 0)
	    {
	      if (CLASSTYPE_CONSTPTR_CONVERSION (basetype) == error_mark_node)
		return error_mark_node;
	      typename = DECL_ORIGINAL_NAME (CLASSTYPE_CONSTPTR_CONVERSION (basetype));
	      return build_type_conversion_1 (xtype, basetype, expr, typename, for_sure);
	    }
	  if (CLASSTYPE_N_BASECLASSES (basetype))
	    basetype = CLASSTYPE_BASECLASS (basetype, 1);
