 ...
 *						- handles swapfiles in /etc/swaptab  10/90 
...
extern void nukeswapfiles (char *);
char	*opticalsizestr;
float	opticalsize = 0.0;
char	*host = 0;		// 0 if dump, hostname if rdump
#else	NeXT_MOD
#ifdef RDUMP
char	*host;
#endif
...
msg("Choose either \'o\' flag for removable disk or \'c\' flag for cartridge\n");
		dumpabort();
	}
	if (optical && density) {
		msg("O (Removable disk) flag specified: Do not give tape density\n");
		dumpabort();
	}
	if (opticalsize == 0)  {
		/*
...
#ifdef	NeXT
	if (dt != 0) {
		char *tmp = rawname(dt->fs_spec);	// a volatile copy
		
		disk = (char *)malloc(strlen(tmp)+1);
		strcpy(disk, tmp);
	}
#else	NeXT
	if (dt != 0)
	...
	msg("estimated %ld %s blocks on %3.2f %s(s).\n",  esize, 
		(optical ? "removable disk" : "tape"), fetapes,
		(optical ? "removable disk" : "tape"));
