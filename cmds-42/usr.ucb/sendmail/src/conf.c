...
setdefaults()
{
	QueueLA = 0;		/* Not supported due to NeXT msg sizes */
	RefuseLA = 12;		/* Load Avg limits high, by default */
#ifdef NeXT_MOD
	SystemProcLimit = 0;	/* Proc counting off, by default. */
#endif
	QueueFactor = 10000;
...
# if NeXT_MOD
	...
	return ((int) ((float)la.tl_avenrun[0] / (float)la.tl_lscale + 0.5));
# else NeXT_MOD
	if (kmem < 0)
	{
