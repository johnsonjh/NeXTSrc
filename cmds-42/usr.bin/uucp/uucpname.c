...
register char *name;
{
	 ...
	 *	Naming Rules:
	 *
	 *	IF /etc/uucp/UUCPNAME simply exists (zero bytes long)
	 *	THEN sitename == "next"
	 *
	 *	IF /etc/uucp/UUCPNAME exists AND there is a name in it
	 *	THEN set the site name to the name just read
	 *
	 *	ELSE set site name == hostname
	 */

	if (s == NULL || *s == '\0')
		if(gethostname(s=Myfullname, sizeof(Myfullname)) == -1) {
			DEBUG(1, "gethostname", _FAILED);
			s = NULL;
			}

	{
		...
		if ((uucpf = fopen("/etc/uucp/UUCPNAME", "r")) != NULL) {
			if(fgets(tmpname, sizeof tmpname, uucpf) == NULL)
				strncpy(s=Myfullname, MYNAME, strlen(MYNAME)+1);
			else 
				strncpy(s=Myfullname, tmpname, strlen(tmpname)+1);
			for(p=Myfullname; *p; p++)
				if(*p == '\n')
					*p = 0;
			}
		if (uucpf != NULL)
		...
		sprintf(DLocal, "%s/D.%.*s", SPOOL, SYSNSIZE, Myfullname);
		sprintf(DLocalX, "%s/D.%.*sX", SPOOL, SYSNSIZE, Myfullname);
		oumask = umask(0);
		if(stat(DLocal, &d)) {
			if(mkdir(DLocal, 0755) < 0)
				logent(DLocal, "CANNOT MKDIR");
			else
				logent(DLocal, "CREATED");
			}
		if(stat(DLocalX, &d)) {
			if(mkdir(DLocalX, 0755) < 0)
				logent(DLocalX, "CANNOT MKDIR");
			else
				logent(DLocalX, "CREATED");
			}
		umask(oumask);
	}
	...
	if (s == NULL || *s == '\0') {
		...
		if (((uucpf = fopen("/etc/uucpname", "r")) == NULL &&
			(uucpf = fopen("/etc/uucp/SITENAME", "r")) == NULL &&
