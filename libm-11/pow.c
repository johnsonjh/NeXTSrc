...
const double static zero=0.0, half=1.0/2.0, one=1.0, two=2.0, negone= -1.0;

/* pow_p(x,y) return x**y for x with sign=1 and finite y */
static double pow_p(x,y)       
double x,y;
{
        double log__L(),exp__E();
...
#ifdef NeXT
	/* This should be OK under the following assumptions:
	 * 1. log(x) returns a result accurate to 1 ULP of double
	 *    for an argument in principal range.
	 * 2. for an argument not in principal range, log(x) becomes
	 *    more accurate, i.e. changing the exponent of x makes
	 *    the result no less accurate.
	 * 3. Intermediate calculations done to extended precision
	 * 4. exp(x) reduces the extended-precision operand to
	 *    principal range with high accuracy
	 * 5. exp(x) in pricipal range is accurate to 1 ULP of double.
	 */
	return exp(log(x)*y);
#else
   ...
   /* if y*log(x) is neither too big nor too small */
	...
	/* end of if log(y*log(x)) > -60.0 */
		...
		/* exp(+- tiny) = 1 with inexact flag */
	        ...
	        errno = ERANGE;
	        if(copysign(one,y)*(n+invln2*t) <zero)
