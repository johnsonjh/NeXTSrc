...
struct search_level
{
  ...
  /* Point to obstack we shoudl return to.  */
...
/* Return the number of levels between type PARENT and type TYPE,
   following the leftmost path to PARENT.  If PARENT
   appears in different places from TYPE's point of view,
   the leftmost PARENT will be the one chosen.
     ...
     int i;
{
  ...
  while (fields)
    {
      for (field = TREE_VALUE (fields); field; field = TREE_CHAIN (field))
	  ...
	  for (tmp = TREE_VALUE (baselink); tmp; tmp = TREE_CHAIN (tmp))
		...
		continue;
	      ...
	      if ((TREE_READONLY (TREE_TYPE (TREE_VALUE (btypes)))
		   == TREE_READONLY (TREE_TYPE (TREE_VALUE (dtypes))))
		  && compparms (TREE_CHAIN (btypes), TREE_CHAIN (dtypes), 1))
		{
		  if (IDENTIFIER_ERROR_LOCUS (DECL_NAME (fndecl)) == NULL_TREE
		      && ! comptypes (TREE_TYPE (TREE_TYPE (tmp)), drettype, 1))
		    {
		      ...
		      SET_IDENTIFIER_ERROR_LOCUS (DECL_NAME (fndecl),
						  TYPE_METHOD_BASETYPE (fndecl));
		    }
		  break;
		}
	    }
     ...
     tree type;
{
  ...
  for (vslots = CLASSTYPE_VFIELDS (type); vslots; vslots = TREE_CHAIN (vslots))
    {
      int normal;
  ...
  for (i = n_baselinks; i > 0; i--)
	...
	CLASSTYPE_SEARCH_SLOT (vbase)
	  = build (PLUS_EXPR, TYPE_POINTER_TO (vbase),
		   vbase_char_star, ASSOC_OFFSET (assoc));
      }
  CLASSTYPE_MARKED3 (type) = 1;
  CLASSTYPE_MARKED4 (type) = 1;
}
     ...
     tree type;
{
  ...
  tree this_vbase_ptr = convert_pointer_to (type, vbase_decl_ptr);
  ...
  if (TYPE_USES_VIRTUAL_BASECLASSES (type))
    {
      ...
      vbase_char_star = build1 (NOP_EXPR, ptr_type_node, decl_ptr);
  ...
  if (TYPE_USES_VIRTUAL_BASECLASSES (type))
    {
      ...
      vbase_char_star = build1 (NOP_EXPR, ptr_type_node, vbase_decl_ptr);
#ifdef sparc
