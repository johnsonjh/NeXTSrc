...
#ifdef NeXT
#include "obstack.h"	/* For obstack_size (). */
#endif  /* NeXT */
...
#ifdef NeXT_OBJC
int doing_objc_thang = 0;
int gen_declaration = 0;
int warn_selector = 0;
#endif  /* NeXT_OBJC */
...
/* Nonzero means optimize flag was given on command line.
   Used by #pragma CC_OPT_RESTORE . */
#ifdef NeXT
int initial_optimize_flag = 0;
#endif  /* NeXT */
...
#ifdef NeXT
/* Nonzero for -fno-common
   place uninitilaized global data in the bss section. */

int flag_no_common = 0;

/* Nonzero for -fno-mach-text-sections:
  Make everything that used to go in the text section really go there.  */

extern int flag_no_mach_text_sections = 0;

/* Nonzero for -fno-auto-overload. */

int flag_no_auto_overload = 0;
#endif  /* NeXT */
...
static struct { char *string; int *variable; int on_value;} f_options[] =
{
  ...
  {"mach-text-sections", &flag_no_mach_text_sections, 0},
  {"auto-overload", &flag_no_auto_overload, 0},
#endif  /* NeXT */
...
#ifdef NeXT_OBJC
      else if (method_name = compiling_a_method())
	fprintf (stderr, "In method `%s'\n", method_name);
#endif  /* NeXT_OBJC */
      else
     ...
     char *name;
{
...
#ifdef NeXT_OBJC
  if (doing_objc_thang)
    init_objc ();	/* Objective-C initialization */
#endif  /* NeXT_OBJC */
  ...
  if (setjmp (toplevel) == 0)
...
#ifdef NeXT_OBJC
    {
      ...
      if (current_objc_implementation_context () != NULL_TREE)
	{
	  warning ("`@end' missing in implementation");
	  finish_class(current_objc_implementation_context ());
	}
    }
  
  if (doing_objc_thang && (errorcount == 0))
     ...
     char **envp;
{
  ...
  for (i = 1; i < argc; i++)
	...
	if (str[0] == 'm')
	  ...
	  optimize = 1, obey_regdecls = 0;
#ifdef NeXT_OBJC
	else if (!strcmp (str, "ObjC"))
	  doing_objc_thang = 1;
	else if (!strcmp (str, "genDecl"))
	  gen_declaration = 1;
	else if (!strcmp (str, "Wselector"))
	  warn_selector = 1;
#endif  /* NeXT_OBJC */
...
#ifndef VMS
  if (print_mem_flag)
#ifdef NeXT	/* NeXT does not support sbrk () .*/
    {
      char s[12];
      int i, j;
      ...
      extern struct obstack permanent_obstack,
			    ...
			    temporary_obstack,
			    momentary_obstack;
      extern char *obstack_code_name[4];
      extern int get_obstack_code (struct obstack *obstack);
      extern unsigned int tree_node_count[NUM_TREE_CODES][4];
      extern unsigned int tree_node_size[NUM_TREE_CODES][4];
      extern char *tree_code_name[];
      
      /* Print number of nodes allocated. */
      ...
      fprintf (stderr, "%20s", "");
      for (i = 0; i < 4; i++)
	fprintf (stderr, "%-10s", obstack_code_name[i]);
      fprintf (stderr, "\n------------------------------------------------------------\n");
      ...
      for (i = 0; i < NUM_TREE_CODES; i++)
	{
	  ...
	  for (j = 0; j < 4; j++)
	    total += tree_node_count[i][j];
	  if (total != 0)
	    {
	      fprintf (stderr, "%-20s", tree_code_name[i]);
	      for (j = 0; j < 4; j++)
	        fprintf (stderr, "%8d  ", tree_node_count[i][j]);
	      fprintf (stderr, "\n");
	    }
	}
      fprintf (stderr, "\n");
      ...
      /* Print memory usage information from each obstack. */

      fprintf (stderr, "Permanent obstack size: %d.\n",
	       obstack_size (&permanent_obstack));
      fprintf (stderr, "Maybe-permanent obstack size: %d.\n",
	       obstack_size (&maybepermanent_obstack));
      fprintf (stderr, "Temporary obstack size: %d.\n",
	       obstack_size (&temporary_obstack));
      fprintf (stderr, "Momentary obstack size: %d.\n",
	       obstack_size (&momentary_obstack));
      fprintf (stderr, "\n");
      
      /* Print memory usage information gathered by malloc (). */

      mstats ();
      fprintf (stderr, "\n");
      
      /* Only print ps info for this process. */

      sprintf (s, "ps v %6d", getpid ());
      system (s);
    }
#else  /* NeXT */
    {
