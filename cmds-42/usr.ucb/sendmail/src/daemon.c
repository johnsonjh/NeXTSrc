...
# endif LOG
	...
	for (;;)
	{
		...
		{
#if	NeXT_MOD
			/* Wait until connection requested. */
			
			do {
				FD_ZERO(&readfs);
				FD_SET(DaemonSocket,&readfs);
			} while ((select(DaemonSocket+1,&readfs,NULL,
				NULL,NULL,NULL) <= 0) && (errno == EINTR));
			
			if (!FD_ISSET(DaemonSocket,&readfs))
			{
				/* Error during select != EINTR, forgerize
				   an accept error (caught just below)
				   and exit inner while loop. */
				
				t = -1;
				break;
			}
			
			/* Wait until load low enough to handle it. */
			
			while (RefuseLA > 0 && getla() > RefuseLA) {
#ifdef DEBUG
			if (tTd(15, 2))
			  printf("Refusing connections, Load = %d\n",getla());
#endif DEBUG
			sendmail_sleep(5);
			}
#endif NeXT_MOD
