...
#ifndef lint
static char sccsid[] = 	"@(#)tar.c	1.3 88/07/05 4.0NFSSRC SMI"; /* from UCB 5.7 4/26/86 */
#endif

/* 
 * HISTORY
 * 06-Mar-89	Doug Mitchell (dmitch) at NeXT
 *	Default tape device = DEFTAPE (from mtio.h).
 * 10-Sep-89	Morris Meyer (mmeyer) at NeXT
 *	Upgraded to NFS 4.0.
 */
...
#if	NeXT
		/*
		 * The Mail app uses exit code 2 to determine if tar
		 * has received a disk full write error.
		 */
		done(5);
#else
		done(2);
