...
/*
 * HISTORY:
 * 25-Apr-90  Gregg Kellogg (gk) at NeXT
 *	Don't rely on the kernel for keeping track of time-zone information.
 */
	...
	char	**argv;
{
	...
	extern char	*optarg;
#if	NeXT
	static char	usage[] = "usage: date [-nu] [yymmddhhmm[.ss]]\n";
#else	NeXT
...
#ifdef DEBUG
	printf ("optarg %s mw %d dsttime %d\n", 
		optarg, tz.tz_minuteswest, tz.tz_dsttime);
#endif DEBUG

#if	NeXT
	/*
	 * Kernel doesn't keep track of timezone information.
	 */
#else	NeXT
...
#if	NeXT
		/*
		 * Machine isn't taking care of timezone for us.
		 */
		tv.tv_sec -= (long)localtime((time_t *)&tv.tv_sec)->tm_gmtoff;
#else	NeXT
	...
	register char	*ap;		/* user argument */
{
	...
	/*
	 * Secs is our local time in GMT timezone, not local timeszone.
	 */
	return(0);
}
