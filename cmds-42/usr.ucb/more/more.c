...
char *filename;
{
	...
	pr("--More--");
	if (filename != NULL) {
	    promptlen += mprintf ("(Next file: %s)", filename);
	}
	else if (!no_intty) {
	    promptlen += mprintf ("(%d%%)", (int)((file_pos * 100) / file_size));
	}
	if (dum_opt) {
...
#define ret(val) retval=val;done++;break
	...
	case 'f':
	    ...
	    if (clreol)
		cleareol ();
	    mprintf ("...skipping %d line", nlines);
	    if (nlines > 1)
		pr ("s\n");
	    else
		pr ("\n");
...
int openfile (char *filename)
{
	...
	switch ((pid = fork())) {
	case 0:
		execl ("/usr/bin/openfile", "openfile", filename, 0);
		_exit(1);
	case -1:
		perror("fork");
		return(-1);
...
int nlines;
{
	...
	case 'f':
		kill_line ();
		if (!no_intty)
			promptlen = mprintf ("\"%s\" line %d", fnames[fnum], Currline);
		else
			promptlen = mprintf ("[Not a file] line %d", Currline);
		fflush (stdout);
		return (-1);
	case 'n':
		if (nlines == 0) {
...
char *filename;
{
	...
	if (lastp)
		...
		if (expand (shell_line, cmdbuf)) {
			kill_line ();
			promptlen = mprintf ("!%s", shell_line);
		}
	}
	fflush (stdout);
	write (2, "\n", 1);
