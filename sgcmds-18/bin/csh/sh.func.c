 ...
 **********************************************************************
 * HISTORY
 * 27-Aug-85  Glenn Marcy (gm0w) at Carnegie-Mellon University
 *	Only call dohash() if havhash is set.
 *
 **********************************************************************
 */
	...
	register char **v;
{
	...
	if ((lp = *v++) == 0)
		lp = "";
#if	CMUCS && !NeXT_MOD
	if (eq(vp, "PATH") || eq(vp, "CPATH") || eq(vp, "LPATH") ||
	    eq(vp, "MPATH") || eq(vp, "EPATH"))
		pathwarn(vp);
#endif	CMUCS
