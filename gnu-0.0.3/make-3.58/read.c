     ...
     char **makefiles;
{
  ...
  {
    char *makefiles = variable_expand ("$(MAKEFILES)");
    char *name;
    unsigned int length;

    /* Set NAME to the start of next token and LENGTH to its length.
       MAKEFILES is updated for finding remaining tokens.  */
    while ((name = find_next_token (&makefiles, &length)) != 0)
...
/* Read file FILENAME as a makefile and add its contents to the data base.
   ...
   cannot determine the default goal, and it's not an error if it doesn't
   exist; 2 => an included makefile: is searched for in the search path.
...
#define record_waiting_files()						      \
  ...
  /* If the makefile wasn't found and it's an included makefile (type 2),
     search the included makefile search path for this makefile.  */

  if (infile == 0 && type == 2 && *filename != '/')
    {
...
#define	word1eq(s, l) 	((p[l] == '\0' || p[l] == ' ' || p[l] == '\t') && \
	  ...
	  || word1eq ("else", 4) || word1eq ("endif", 5))
	{
	  ...
	  if (i >= 0)
	    ...
	    fatal ("%s:%u: invalid syntax in conditional", filename, lineno);
	}
      if (ignoring)
	continue;
      else if (lb.buffer[0] == '\t')
...
#undef	word1eq
      ...
      else
	{
	  ...
	  if (*p2 == '\0')
	    ...
	    fatal ("%s:%u: missing target name", filename, lineno);
	  ...
	  if (p != 0)
	    {
	      ...
	      ++p2;
	      if (target == 0)
		fatal ("%s:%u: missing target pattern", filename, lineno);
	      else if (target->next != 0)
		fatal ("%s:%u: multiple target patterns", filename, lineno);
	      pattern = target->name;
	      pattern_percent = find_percent (pattern);
	      if (pattern_percent == 0)
		fatal ("%s:%u: target pattern contains no `%%'",
		       filename, lineno);
	    }
	  else
	    pattern = 0;
  ...
  if (ignoring)
    fatal ("%s:%u: missing `endif'", filename, lineno);
     ...
     char *filename;
{
  ...
  while (!feof (infile))
	  ...
	  && !strncmp (p, "endef", 5))
	{
	  ...
	  collapse_line (p);
	  if (*next_token (p) != '\0')
	    error ("%s:%u: Extraneous text after `endef' directive",
		   filename, lineno);
	  /* Define the variable.  */
	  definition[idx] = '\0';
  ...
  /* No `endef'!!  */
  fatal ("%s:%u: missing `endef', unterminated `define'", filename, lineno);
  return 0;
}

     ...
     unsigned int lineno;
{
  ...
  if (*cmdname == 'e')
    {
      if (*line != '\0')
	error ("%s:%u: Extraneous text after `%s' directive",
	       filename, lineno, cmdname);
      /* "Else" or "endif".  */
      if (conditionals->if_cmds == 0)
	fatal ("%s:%u: extraneous `%s'\n", filename, lineno, cmdname);
  ...
  for (; filenames != 0; filenames = nextf)
      ...
      if (!two_colon)
	{
	  ...
	  if (cmds != 0 && cmds == f->cmds)
	    error ("%s:%u: target `%s' given more than once in the same rule.",
		   filename, lineno, f->name);
	  /* Check for two single-colon entries both with commands.
	     Check is_target so that we don't lose on files such as .c.o
	     whose commands were preinitialized.  */
	  else if (cmds != 0 && f->cmds != 0 && f->is_target)
	    fatal ("%s:%u: commands for target `%s' were \
already specified at %s:%u",
		   cmds->filename, cmds->lineno,
		   f->name, f->cmds->filename, f->cmds->lineno);
	  f->is_target = 1;
     ...
     char *pattern;
{
  ...
  while ((p = index (p, '%')) != 0)
	...
	len = pattern_len - (p - pattern);
	bcopy (&p[i / 2], &p[i], len - (i / 2));
	p -= i / 2;
	p[len - 1] = '\0';
	if (i % 2 == 0)
  ...
  for (old = chain; old != 0; old = nexto)
      ...
      if (*old->name == '~')
	{
	  if (old->name[1] == '/' || old->name[1] == '\0')
	      ...
	      if (!is_variable)
		{
	      ...
	      if (home_dir == 0)
		{
		  unsigned int len = strlen (old->name + 1);
		  bcopy (old->name + 1, old->name, len);
		  old->name[len] = '\0';
		}
	      else
		{
      ...
      if (glob_pattern_p (old->name))
	{
	  ...
	  if (vec != (char **) -1)
		...
		++i;
	      length = i;
	      qsort ((char *) vec, length, sizeof (char *), alpha_compare);
	      for (i = length; i > 0; --i)
		{
