...
#endif  /* NeXT */
  {".s",
   "%{!S:cpp %{nostdinc} %{C} %{v} %{D*} %{U*} %{I*} %{M} %{MM} %{trigraphs} \
        -undef -D__GNU__ -D__ASSEMBLER__ -$ %p %P\
        %c %{O:-D__OPTIMIZE__} %{traditional} %{pedantic}\
	%{Wcomment*} %{Wtrigraphs} %{Wall} %C\
        %i %{!M:%{!MM:%{!E:%{!pipe:%g.s}}}}%{E:%{o*}}%{M:%{o*}}%{MM:%{o*}} |\n\
    %{!M:%{!MM:%{!E:as %{R} %{j} %{J} %{h} %{d2} %a \
                    %{c:%{o*}%{!o*:-o %w%b.o}}%{!c:-o %d%w%b.o}\
		    %{!pipe:%g.s}\n }}}}"},
  /* Mark end of table */
  {0, 0}
...
char *link_spec = "%{!c:%{!M:%{!MM:%{!E:%{!S:ld -r -o %g.R %l\
 %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\
 %{y*} %{!A:%{!nostdlib:%S}} \
 %{L*} %o %{!nostdlib:%{g:-lg} %L}\n }}}}}";
#else
/* Here is the spec for running the linker, after compiling all files.  */
char *link_spec = "%{!c:%{!M:%{!MM:%{!E:%{!S:ld %{o*} %l\
 %{A} %{d} %{e*} %{N} %{n} %{r} %{s} %{S} %{T*} %{t} %{u*} %{X} %{x} %{z}\
 %{y*} %{!A:%{!nostdlib:%S}} \
 %{L*} %o %{!nostdlib:%{g:-lg} %L}\n }}}}}";
#endif
...
#ifdef NeXT
#ifndef STANDARD_SYS_PREFIX
#define STANDARD_SYS_PREFIX "/bin/"
#endif /* !defined STANDARD_SYS_PREFIX */

char *standard_sys_prefix = STANDARD_SYS_PREFIX;
#endif  /* NeXT */
...
#ifdef NeXT
  if (strlen (standard_sys_prefix) > size)
    size = strlen (standard_sys_prefix);
#endif  /* NeXT */
  size += strlen (prog) + 1;
  if (machine_suffix)
  ...
  if (!win)
    {
      if (machine_suffix)
	{
	  strcpy (temp, standard_sys_prefix);
	  strcat (temp, machine_suffix);
	  strcat (temp, prog);
	  win = (access (temp, X_OK) == 0);
	}
...
struct switchstr
{
  ...
  char *part1;
  char *part2;
#ifdef NeXT	/* Add support for switches with up to three arguments */
  char *part3;
  char *part4;
#endif  /* NeXT */
  int valid;
  ...
  for (i = 1; i < argc; i++)
      ...
      if (argv[i][0] == '-' && argv[i][1] != 'l')
	{
	  ...
	  switch (c)
		...
		i++;
#ifdef NeXT
	      else if (WORD_SWITCH_TAKES_TWO_ARGS (p))
		i += 2;
	      else if (WORD_SWITCH_TAKES_THREE_ARGS (p))
		i += 3;
#endif  /* NeXT */
	    }
	}
      ...
      if (argv[i][0] == '-' && argv[i][1] != 'l')
	{
...
#ifdef NeXT		/* disable -b since it interferes with -bsd */
	  if (c == 'B')
#else  /* NeXT */
	  if (c == 'B' || c == 'b')
#endif  /* NeXT */
	  ...
	  if ((SWITCH_TAKES_ARG (c) && p[1] == 0)
	      || WORD_SWITCH_TAKES_ARG (p))
	    {
	      ...
	      switches[n_switches].part3 = 0;
	      switches[n_switches].part4 = 0;
	    }
	  else if (WORD_SWITCH_TAKES_TWO_ARGS (p))
	    {
	      switches[n_switches].part2 = argv[++i];
	      switches[n_switches].part3 = argv[++i];
	      switches[n_switches].part4 = 0;
	    }
	  else if (WORD_SWITCH_TAKES_THREE_ARGS (p))
	    {
	      switches[n_switches].part2 = argv[++i];
	      switches[n_switches].part3 = argv[++i];
	      switches[n_switches].part4 = argv[++i];
	    }
#else  /* NeXT */
	  if ((SWITCH_TAKES_ARG (c) && p[1] == 0)
	      || WORD_SWITCH_TAKES_ARG (p))
		  ...
		  if (! strncmp (y, "-D", 2))
		    {
		      ...
		      *x++ = 'D';
#ifdef NeXT
		      /* Only convert non-standard pre-defines. */
		      if (y[2] == '_' && (y[3] == '_' || isupper (y[3])))
			is_standard = 1;
		      else
			{
			  is_standard = 0;
...
#ifdef NeXT
  if (switches[switchnum].part3 != 0)
    {
      do_spec_1 (switches[switchnum].part3, 1);
      do_spec_1 (" ", 0);
    }
  if (switches[switchnum].part4 != 0)
    {
      do_spec_1 (switches[switchnum].part4, 1);
      do_spec_1 (" ", 0);
    }
#endif  /* NeXT */
  ...
  if (vflag)
    {
      ...
      fprintf (stderr,
	       "NeXT Release 2.0 (v%s) -- GNU version %s\n",
	       VERS_NUM,
	       version_string);
#else  /* NeXT */
