...
#define ONEHOUR		3600
int backoff [NUMRETRIES] = {5, 10, 15, 20, 30, 40, 50, 60, 120, 180};
#endif	NeXT
	...
	char *name, *fromaddr;
{
...
#if	NeXT
	/*
	 * One hour should be enough for the Mailapp to get a huge
	 * attachment from the spoolfile and drop it into the user's local
	 * mail directory. This makes the assumption that if the mailhost
	 * reboots in the middle of the Mailapp taking the mail from the
	 * spool directory (after creating a lockfile), that the machine
	 * will come up within an hour.
	 */
	sprintf (lockfile, "%s%s.lock", maildir, name);
	if (stat (lockfile, &lockstat) != -1)  {
		time (&now);
		if ((now - lockstat.st_ctime) > ONEHOUR)  {
			unlink (lockfile);
			syslog (LOG_ERR, 
				"binmail unlinking %d minute old lockfile",
				(now - lockstat.st_ctime) / 60);
		}
	}
	for (try = 0; try < NUMRETRIES; try++)  {
		lockfd = open (lockfile, O_RDWR | O_CREAT | O_EXCL, MAILMODE);
		if (lockfd == -1)  {
#if	DEBUG
			syslog (LOG_INFO, "binmail - backoff %d (%d seconds)",
				try, backoff [try]);
#endif	DEBUG
			sleep (backoff[try]);
		}
	...
	if (try == NUMRETRIES) {
		syslog (LOG_ERR, "binmail ignoring lockfile\n");
	}
	if (!safefile(file))  {
		unlink (lockfile);
		return(0);
