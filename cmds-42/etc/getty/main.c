...
#if	NeXT
#else	NeXT
		/* NeXT: serial driver already does this */
		/*
...
#if	NeXT
	/*
	 * Give remote a second to get ready.  Otherwise initial
	 * message and login prompt lost or garbled.
	 */
	sleep(2);
#endif	NeXT
	for (;;) {
...
#if	NeXT
		if (OPset || EPset || APset || ZPset)
			APset++, OPset++, EPset++, ZPset++;
#else	NeXT
...
#ifdef DEBUG
			printf("getty flags = 0x%x\n", tmode.sg_flags);
			dumpenv("environment from init", environ);
#endif DEBUG
			for (i = 0; environ[i] != (char *)0; i++)
				env[i] = environ[i];
			makeenv(&env[i]);
#ifdef DEBUG
			dumpenv("environment from getty", env);
#endif DEBUG
