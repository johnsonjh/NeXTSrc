...
#ifdef NEW_MAD
#ifdef V6
	mode = nonexist ? 0644 : (stbuf.st_mode & ~S_IFMT) | 0644;
#else
	mode = nonexist ? 0666 : (stbuf.st_mode & ~S_IFMT) | 0666;
#endif
#endif NEW_MAD
	switch (c) {
...
*/
#ifdef NEW_MAD
		if (dofname == 2 && !nonexist) {
			nonexist = lstat(file, &stbuf);
			if ((stbuf.st_mode & S_IFMT) == S_IFLNK) {
				char *cp;
				char *lnresolve();

				if (unlink(cp = lnresolve(file)) < 0)
					serror(" Can't unlink| Can't unlink %s", cp);
			} else if (unlink(file) < 0)
				serror(" Can't unlink| Can't unlink %s", file);
		}
		io = creat(file, mode);
#else
#ifdef V6
