...
static long lookup_literal_pointer(
...
struct section_map *section_map)
{
    ...
    long section_value, input_section_offset, merged_section_offset, offset;
    ...
    if(s->size % 4 != 0){
	error_with_cur_obj("literal pointer section (%0.16s,%0.16s) size is "
			   "not a multiple of 4 bytes",s->segname, s->sectname);
	return;
    }
    nliterals = s->size / 4;
    if(s->nreloc != nliterals){
	error_with_cur_obj("literal pointer section (%0.16s,%0.16s) does not "
...
#ifdef mc68000
    /*
     * On real machines the alignment of: cur_obj->obj_addr + s->offset would
     * have to be checked with respect to the alignment of a long make sure we
     * don't cause a fault here.  But for now it works on the mc68000 machines.
     */
    literals = (long *)(cur_obj->obj_addr + s->offset);
#else !mc68000
#error "literal_pointer_merge() must be updated for non-mc68000 machines"
#endif
	...
	if((relocs[i].r_address & R_SCATTERED) != 0){
		...
		error_with_cur_obj("r_value (0x%x) field of relocation "
		    ...
		    r_value, i, s->segname, s->sectname);
		continue;
	    }
	}
	else{
	 ...
	 */
	if(r_address < 0 || r_address >= s->size){
	...
	if(r_address % 4 != 0){
	    error_with_cur_obj("r_address (0x%x) field of relocation entry "
		"%d in literal pointer section (%0.16s,%0.16s) is not a "
		"multiple of 4", r_address, i, s->segname, s->sectname);
	    continue;
	}
	if(r_length != 2){
	    error_with_cur_obj("r_length (0x%x) field of relocation entry "
		"%d in literal pointer section (%0.16s,%0.16s) is not 2 (long)",
		r_length, i, s->segname, s->sectname);
	    continue;
	}
	if(r_pcrel != 0){
	    error_with_cur_obj("r_pcrel (0x%x) field of relocation entry "
		"%d in literal pointer section (%0.16s,%0.16s) is not 0 "
		"(FALSE)", r_pcrel, i, s->segname, s->sectname);
	    continue;
	}
	/*
	...
	if(r_extern){
		...
		if(nlists[r_symbolnum].n_type != (N_EXT | N_UNDF)){
		    error_with_cur_obj("r_symbolnum (%d) field of external "
			"relocation entry %d in section (%0.16s,%0.16s) "
			"refers to a non-undefined symbol", r_symbolnum, i,
			s->segname, s->sectname);
		    continue;
		}
		print_obj_name(cur_obj);
		fatal("internal error, in merge_literal_pointers() symbol "
		      "index %d in not in undefined map", r_symbolnum);
	    }
		...
		error_with_cur_obj("r_symbolnum (%d) field of external "
		    "relocation entry %d in section (%0.16s,%0.16s) refers to "
	     ...
	     */
	    if(merged_symbol->nlist.n_type == (N_EXT | N_ABS)){
		error_with_cur_obj("r_symbolnum (%d) field of external "
		    "relocation entry %d in section (%0.16s,%0.16s) refers to "
		...
		if(literal_ms->s.flags != S_CSTRING_LITERALS &&
		   literal_ms->s.flags != S_4BYTE_LITERALS &&
		   literal_ms->s.flags != S_8BYTE_LITERALS){
		    error_with_cur_obj("exteral symbol (%s) for external "
			"relocation entry %d in section (%0.16s,%0.16s) refers "
		...
		if(section_value < literal_s->addr ||
		   section_value > literal_s->addr + literal_s->size){
			...
			"the section (%0.16s,%0.16s) it is defined in", 
		...
		error_with_cur_obj("r_symbolnum (0x%x) field of relocation "
		    "entry %d in literal pointer section (%0.16s,%0.16s) is "
	    ...
	    if(r_scattered == 0){
		offset = 0;
		section_value = literals[r_address/4];
		if(section_value < literal_s->addr ||
		   section_value > literal_s->addr + literal_s->size){
		    error_with_cur_obj("literal pointer (0x%x) in section "
			"(%0.16s,%0.16s) at address 0x%x does not point into "
			"its (%0.16s,%0.16s) section as refered to by its "
			"r_symbolnum (0x%x) field in relocation entry %d as "
			"it should", section_value, s->segname, s->sectname,
			s->addr + r_address, literal_s->segname,
			literal_s->sectname, r_symbolnum, i);
		    continue;
		}
		if(literal_ms->s.flags != S_CSTRING_LITERALS &&
		   literal_ms->s.flags != S_4BYTE_LITERALS &&
		   literal_ms->s.flags != S_8BYTE_LITERALS){
			...
			"entry %d in literal pointer section (%0.16s,%0.16s) "
			"refers to section (%0.16s,%0.16s) which is not a "
			"literal section", r_symbolnum, i, s->segname,
		...
		offset = literals[r_address/4] - r_value;
		...
		if(literal_ms->s.flags != S_CSTRING_LITERALS &&
		   ...
		   literal_ms->s.flags != S_8BYTE_LITERALS){
			...
			"entry %d in literal pointer section (%0.16s,%0.16s) "
			"refers to section (%0.16s,%0.16s) which is not a "
			"literal section", r_value, i, s->segname,
			s->sectname, literal_ms->s.segname,
			literal_ms->s.sectname);
		    continue;
		}
	...
	if(fine_relocs[r_address/4].output_offset != -1){
		...
		"(%0.16s,%0.16s)", s->addr + r_address, r_address, s->segname,
		s->sectname);
	    continue;
	}
	...
	fine_relocs[r_address/4].output_offset =
		lookup_literal_pointer(merged_symbol, literal_ms,
				       merged_section_offset, offset, data, ms);
    }
}
...
struct merged_section *ms)
{
    long i, j, line_number, line_length, max_line_length, char_pos,
	...
	for(i = 0; i < ms->order_size; i++){
		...
		error("format error in -sectorder file: %s line %d for section "
		      "(%0.16s,%0.16s) (missing ':' after segment name)",
	    ...
	    if(i >= ms->order_size || ms->order_addr[i] == '\0'){
		error("format error in -sectorder file: %s line %d for section "
		      "(%0.16s,%0.16s) (missing ':' after section name)",
		      ms->order_filename, line_number, ms->s.segname,
		      ms->s.sectname);
		continue;
	    }
	    ...
	    if(literal_ms == NULL){
		error("error in -sectorder file: %s line %d for section "
		      "(%0.16s,%0.16s) (specified section (%s,%s) is not "
		      "loaded objects)", ms->order_filename, line_number,
		      ms->s.segname, ms->s.sectname, segname, sectname);
	    }
	    else{
		switch(literal_ms->s.flags){
		case S_CSTRING_LITERALS:
		    ...
		    (void)lookup_literal_pointer(NULL, literal_ms,
					    merged_section_offset, 0, data, ms);
		    break;
		case S_4BYTE_LITERALS:
			...
			error("error in -sectorder file: %s line %d for section"
			      " (%0.16s,%0.16s) (missing hex number for "
		...
		case S_8BYTE_LITERALS:
			...
			if(get_hex_from_sectorder(ms, &i, &(literal8.long1),
			    ...
			    error("error in -sectorder file: %s line %d for "
				  "section (%0.16s,%0.16s) (missing second hex "
			...
			error("error in -sectorder file: %s line %d for section"
			      " (%0.16s,%0.16s) (missing first hex number for "
			      "specified 8 byte literal section (%s,%s))",
			      ms->order_filename, line_number,
			      ms->s.segname, ms->s.sectname, segname, sectname);
		    }
		    break;
		default:
		    error("error in -sectorder file: %s line %d for section "
			  "(%0.16s,%0.16s) (specified section (%s,%s) is not a "
...
#ifdef mc68000
	/*
	 * On real machines the alignment of: output_addr + ms->s.offset would
	 * have to be checked with respect to the alignment of a long make sure
 	 * we don't cause a fault here.  But for now it works on the mc68000
	 * machines.
	 */
	output_pointer = (long *)(output_addr + ms->s.offset);
#else !mc68000
#error "literal_pointer_output() must be updated for non-mc68000 machines"
#endif
	for(p = &(data->literal_pointer_blocks);
	    *p ;
	    p = &(literal_pointer_block->next)){
...
#ifdef mc68000
	    /*
	     * On real machines the alignment of: output_addr + ms->s.reloff
	     * would have to be checked with respect to the alignment of a
	     * struct relocation_info and a struct scattered_relocation_info
	     * to make sure we don't cause a fault here.  But for now it works
	     * on the mc68000 machines.
	     */
	    reloc = (struct relocation_info *)(output_addr + ms->s.reloff);
	    sreloc = (struct scattered_relocation_info *)
					      (output_addr + ms->s.reloff);
#else !mc68000
#error "literal_pointer_output() must be updated for non-mc68000 machines"
#endif
	    r_address = 0;
		...
		*p ;
	    ...
	    output_flush(ms->s.reloff,
			 (char *)reloc - (char *)(output_addr + ms->s.reloff));
	}
#endif !defined(RLD)
...
char *indent)
{
	...
	for(p = &(data->literal_pointer_blocks);
	    *p ;
	    ...
	    for(i = 0; i < literal_pointer_block->used; i++){
		if(literal_pointers[i].merged_symbol != NULL){
		    print("%s    symbol %s offset %d\n", indent,
			  literal_pointers[i].merged_symbol->nlist.n_un.n_name,
			  literal_pointers[i].offset);
		}
		else{
		    print("%s    section (%0.16s,%0.16s) section_offset %d "
			  "offset %d\n", indent,
...
struct merged_section *ms)
{
	if(data == NULL)
	    return;
	print("literal pointer section (%0.16s,%0.16s) contains:\n",
	      ms->s.segname, ms->s.sectname);
	print("    %d merged literal pointers\n", ms->s.size / 4);
	print("    from %d files and %d total literal pointers from those "
