     ...
     tree strings;
{
  ...
  if (TREE_CHAIN (strings))
	...
	{
	  if (TREE_TYPE (t) == wide_char_array_type_node)
	    {
	 ...
	 which makes them take more space.  */
      if (wide_flag)
	length = length * UNITS_PER_WIDE_CHAR + wide_length;

      p = (char *) savealloc (length);

      /* Copy the individual strings into the new combined string.
	 If the combined string is wide, convert the chars to wide chars
      ...
      for (t = strings; t; t = TREE_CHAIN (t))
	{
	  ...
	  if ((TREE_TYPE (t) == wide_char_array_type_node) == wide_flag)
	    {
	  ...
	  else
	    {
	      int i;
	      for (i = 0; i < len; i++)
		((wchar_t *) q)[i] = TREE_STRING_POINTER (t)[i];
	      q += len * UNITS_PER_WIDE_CHAR;
	    }
	}
      *q = 0;
  ...
  else
    {
      ...
      if (TREE_TYPE (value) == wide_char_array_type_node)
	wide_flag = 1;
    }
  ...
  if (warn_write_strings)
    {
      tree elements
	= build_type_variant (wide_flag ? wide_char_array_type_node : char_type_node,
			      1, 0);
      TREE_TYPE (value)
...
#endif  /* NeXT_PARANOIA */
    TREE_TYPE (value)
      = build_array_type (wide_flag ? wide_char_array_type_node : char_type_node,
			  build_index_type (build_int_2 (length - 1, 0)));
#ifdef NeXT_PARANOIA
...
static wchar_t *wide_buffer;	/* Pointer to wide-string buffer.
...
init_lex ()
{
  ...
  max_wide = 40;
  wide_buffer = (wchar_t *) xmalloc ((max_wide + 1) * sizeof (wchar_t));
...
/*  yylloc.first_line = lineno; */
  ...
  switch (c)
	...
	*--p = 0;
	...
	if (wide_flag) width = TYPE_PRECISION (wide_char_type_node);
	...
	/* If char type is signed, sign-extend the constant.  */
	  {
	    unsigned = wide_flag ? TREE_UNSIGNED (wide_char_type_node)
	...
	while (c != '"' && c >= 0)
	  {
		...
		if (c < 0)
		  goto skipnewline;
		if (c >= (1 << TYPE_PRECISION (wide_flag ? wide_char_type_node
							 : char_type_node)))
		  warning ("escape sequence out of range for character");
	      }
	    else if (c == '\n')
		...
		if (widep == wide_buffer + max_wide)
		  {
		    ...
		    max_wide *= 2;
		    wide_buffer = (wchar_t *) xrealloc (wide_buffer,
						(max_wide + 1) * sizeof (wchar_t));
		    widep = wide_buffer + n;
		  }
		*widep++ = c;
	      }
	...
	if (wide_flag)
	  {
	    /* If this is a L"..." wide-string, make a vector
	       of the wchar_t's in wide_buffer.  */
	    *widep = 0;
	    /* We have not implemented the case where `int'
	       on the target and on the execution machine differ in size.  */
	    if (TYPE_PRECISION (wide_char_type_node)
		!= sizeof (int) * BITS_PER_UNIT)
	      abort ();
	    yylval.ttype = build_string ((widep - wide_buffer) * sizeof (wchar_t),
					 wide_buffer);
	    TREE_TYPE (yylval.ttype) = wide_char_array_type_node;
	  }
	else
	  {
	    *p = 0;
