#ifndef lint
static char *sccsid = "@(#)quot.c	1.2 88/03/15 4.0NFSSRC";	/* from 4.11 (Berkeley) 85/09/09 */
#endif
...
quotall()
{
	...
	fstab = setmntent(NULL, "r");
	if (fstab == NULL) {
		fprintf(stderr, "quot: no filesystem table\n");
		exit(1);
	}
	...
	register struct dinode *ip;
{
	...
	if (!hflg)
		...
		printf ("frags %d fs_frag %d blks %d size %d\n",
			frags, sblock.fs_frag, blks, size);
	}
	if (cflg) {
