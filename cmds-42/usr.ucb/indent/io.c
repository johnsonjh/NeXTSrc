...
{				/* dump_line is the routine that actually
	...
	while (--n_real_blanklines >= 0)
	    myputc('\n', output);
	n_real_blanklines = 0;
	if (ps.ind_level == 0 && !ps.in_message_decl)	/* 12/18/86 wrp added */
		...
		*e_com = 0;
		...
		while (target <= 0)
			...
			target++, s_com++;
		    	...
		    	/* 12/18/86 wrp changed to allow for var tabsize */
			target--;
...
/* 12/18/86 wrp added ps.in_message_decl to following line */
    ps.ind_stmt = (ps.in_stmt && !ps.in_decl) || ps.in_message_decl; /* next 
    					 * line should be
...
compute_code_target() {
	...
	if (ps.in_message_decl)		/* 12/18/86 wrp added for objc */
    ...
    int         target;		/* position we want it at */
{
	...
	curr = current;
	/* 12/18/86 wrp changed to allow for variable tabsize */
	while ((tcur = ((curr - 1) - ((curr - 1) % tabsize)) + tabsize + 1) <= target) {
	    myputc('\t', output);
	    curr = tcur;
	}
	while (curr++ < target)
	    myputc(' ', output);	/* pad with final blanks */
    }
    return (target);
};
