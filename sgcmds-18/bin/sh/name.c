...
static int namwalk(register struct namnod *np);
...
char	*v;
{
	...
	{
		if (n == &pathnod || eq(n->namid, shell))	/* DAG -- made string sharable */
...
struct namnod *n;
{
	...
	{
		prs_buff(n->namid);
		prs_buff(sfuncstr);	/* DAG -- made string sharable */
		prf(n->namenv);
		prs_buff(efnlstr);	/* DAG -- made string sharable */
	}
	else if (s = n->namval)
	{
		prs_buff(n->namid);
...
register struct namnod *n;
{
	...
	p = movstr(eqlstr, p);	/* DAG -- made string sharable */
	p = movstr(n->namval, p);
...
setup_env()
{
	...
	while (*e)
	    setname(make(*e++), N_ENVNAM);  /* don't free env are on stack! */
}
	...
	register char 	*name;
{
	...
	if (n = findnam(name))
	{
		...
		if ((flags & rshflg) && eq(name, shell))	/* DAG -- made string sharable */
