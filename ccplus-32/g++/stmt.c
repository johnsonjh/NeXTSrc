...
/* Cleanup lists are required for binding levels regardless of whether
   that binding level has cleanups or not.  This node satisfies that
   need for all binding levels.  */
...
extern void balance_case_nodes ();
extern void emit_case_nodes ();
extern void group_case_nodes ();
extern void emit_jump_if_reachable ();

  ...
  union
      ...
      struct
	{
	  ...
	  /* List of cleanup-lists of blocks containing this block,
	     ...
	     ordered innermost containing block first, once there are
	     any cleanups at all.
...
struct goto_fixup
{
  ...
  /* List of lists of cleanup expressions to be run by this goto.
     ...
     once there are any cleanups at all.
...

/* Return truth-value of whether there are any cleanups from
   the current binding contour to the end of the current function's
   binding contours.  */
int
     ...
     tree exp;
{
  switch (TREE_CODE (exp))
    ...
    case COND_EXPR:
      ...
      /* These kinds of exprs are really stmts.  What to do?  */
    case LOOP_STMT:
    case LET_STMT:
    case IF_STMT:
      return 0;
  ...
  while (decl_elts)
    {
      ...
      enum machine_mode tmode = TYPE_MODE (TREE_TYPE (decl_elt));

      if (GET_CODE (x) == MEM)
	{
	  /* @@ calling `change_address' means that we cannot
	     be at top-level, since `memory_address' might try
	     to kick this address into a register, which won't
	     work.  Will this work?  */
	  rtx new = gen_rtx (MEM, tmode, XEXP (x, 0));
	  DECL_RTL (decl_elt) = new;
	  MEM_VOLATILE_P (new) = MEM_VOLATILE_P (x);
	  RTX_UNCHANGING_P (new) = RTX_UNCHANGING_P (x);
	  MEM_IN_STRUCT_P (new) = MEM_IN_STRUCT_P (x);
	}
      else
	DECL_RTL (decl_elt) = gen_rtx (SUBREG, tmode, x, 0);
...
/* See case.c for CASE-handling code.  */

     ...
     tree fndecl;
{
  ...
  for (parm = DECL_ARGUMENTS (fndecl); parm; parm = TREE_CHAIN (parm))
	...
	= (TREE_CODE (TREE_TYPE (parm)) == ARRAY_TYPE
      ...
      /* Extra bytes to add in after parameter is assigned, in
	 case where argument cannot be assigned an offsetted
	 location.  For example, BLKmode parameters cannot be
	 other than on a word boundary (no matter the size)
	 because `access_parm_map' does not know how to handle
	 that case.  */
      int extra = 0;
      ...
      if (nominal_mode == BLKmode)
	{
	  ...
	  if (GET_CODE (entry_parm) == REG)
	    ...
	    {
	      /* If this function uses varargs, and `__builtin_saveregs'
		 can clobber this stack location, then protect it.  */
	      rtx pseudo_parm;
#ifdef FUNCTION_INCOMING_ARG
	      pseudo_parm
		= FUNCTION_INCOMING_ARG (args_so_far, SImode,
					 integer_type_node, 1);
#else
	      pseudo_parm
		= FUNCTION_ARG (args_so_far, SImode,
				integer_type_node, 1);
#endif
	      if (pseudo_parm && GET_CODE (pseudo_parm) == REG)
		{
		  push_to_sequence (save_from_saveregs);
		  move_block_to_reg (REGNO (pseudo_parm), stack_parm,
				     int_size_in_bytes (TREE_TYPE (parm))
				     / UNITS_PER_WORD);
		  save_from_saveregs = get_insns ();
		  end_sequence (0);
		}
	    }
     ...
     tree subr;
{
  ...
  current_function_name = (char *)lang_printable_name (subr);
  ...
  /* We are not currently within any block, conditional, loop or case.
     @@ No longer true.  We are within the block for the parms.  */
  block_stack = 0;
  loop_stack = 0;
  case_stack = 0;
  cond_stack = 0;
  ...
  /* No parameters to protect from `__builtin_saveregs' yet.  */
  save_from_saveregs = 0;
...
/* ??? Nobody seems to emit the cleanup_label and the cleanups themselves.
   ...
   !!! Not true.  finish_function does this is cplus-decl.c.  */
...
#if 1
      /* If this works, leave it in.  */
      /* This is not really the function value,
	 and it confuses integrate.c around line 1211.  */
