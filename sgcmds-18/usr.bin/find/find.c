/* @(#)find.c	1.3 87/07/16 3.2/4.3NFSSRC */
#ifndef	lint
...
struct anode *e3() { /* parse parens and predicates */
#if	NeXT
	int exeq(), ok(), glob(),  mtime(), atime(), st_ctime(), user(), 
		charsize(),
#else
	int exeq(), ok(), glob(),  mtime(), atime(), user(),
#endif	NeXT
		group(), size(), perm(), links(), print(),
		type(), ino(), cpio(), newer(), fstype(), prune(),
	...
	if(EQ(a, "-name"))
		...
		return(mk(st_ctime, (struct anode *)atoi(b), (struct anode *)s));
#endif	NeXT
	else if(EQ(a, "-mtime"))
		...
		if (b[strlen(b) - 1] == 'c')
			return(mk(charsize, (struct anode *)atoi(b), 
					(struct anode *)s));
		else
			return(mk(size, (struct anode *)atoi(b), 
					(struct anode *)s));
	}
#else
	} else if(EQ(a, "-size"))
...
#if	NeXT
		fprintf(stderr,"find: cannot chdir to %s: %s\n", 
