 ...
 * nm_init.c
 *
 * $Source: /os/osdev/LIBS/libs-7/etc/nmserver/server/RCS/nm_init.c,v $
 *
 */

#ifndef	lint
char nm_init_rcsid[] = "$Header: nm_init.c,v 1.1 88/09/30 15:40:50 osdev Exp $";
#endif not lint
...
#if	NeXT && 0
	/* after initializing everything (i.e., opening protected sockets)
	 * lower our privileges.  This allows the task_secure mechanism 
	 * (part of setuid execution) to function securely.  Note that 
	 * we only set the effective uid in case we need to become root 
	 * again later (although it would open up a window without security).
	 */
	(void) seteuid(-2);
#endif	NeXT && 0
    return success;
