...
#ifdef NeXT_MOD
	bzero(&skun, sizeof(skun));
#endif NeXT_MOD
	skun.sun_family = AF_UNIX;
	(void) strncpy(skun.sun_path, LogName, sizeof skun.sun_path);
	funix = socket(AF_UNIX, SOCK_DGRAM, 0);
	if (funix < 0 || bind(funix, (struct sockaddr *) &skun,
	    sizeof(skun.sun_family)+strlen(skun.sun_path)) < 0 ||
...
#ifdef NeXT_MOD
	(void) sprintf(line, "%.15s mach: ", ctime(&now) + 4);
#else
