...
{	register int i, j, nlen = 0;
	...
	u_scandir(CMDSDIR, "C.", CMDSLEN, NULL, CMDTYPE);
	u_scandir(DATADIR, "D.", DATALEN, NULL, DATTYPE);
	u_scandir(XEQTDIR, "X.", XEQTLEN, 'X', XEQTYPE);
	getstst(SPOOL);
	time(&curtime);
	for(i=0; i<sndx; ++i)
...
u_scandir(dnam, prfx, flen, fchr, type)
char *dnam, *prfx, fchr;
{
