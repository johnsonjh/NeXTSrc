...
static tree objc_object_id, _OBJC_SELECTOR_REFERENCES_id;
...
extern struct obstack permanent_obstack, *current_obstack;
...
extern void poplevel(int keep, int reverse, int functionbody);
...
{
  return xref_tag(RECORD_TYPE, interface->class.my_name);
}
...
static void synth_module_prologue()
{
  ...
  class_type = groktypename(build_tree_list(build_tree_list(NULLT, 
			xref_tag(RECORD_TYPE, get_identifier(TAG_CLASS))),
			build(INDIRECT_REF,NULLT,NULLT)));
...
#ifdef NeXT_SELS_R_INTS
  /* static unsigned int selTransTbl[] = { 1, 2, ... }; */
  decl_specs = tree_cons(NULLT, ridpointers[RID_UNSIGNED], sc_spec);
  decl_specs = tree_cons(NULLT, ridpointers[RID_INT], decl_specs);
  expr_decl = build_nt(ARRAY_REF, _OBJC_SELECTOR_REFERENCES_id, NULLT);
#endif
...
tree getArgTypeList(tree meth, int context, int superflag)
{
  ...
  for (akey = meth->method.sel_args; akey; akey = TREE_CHAIN(akey))
    ...
    chainon(arglist,build_tree_list(NULLT,arg_type));
    }
...
#ifdef NeXT_SELS_R_INDIRECT
  chainon(params,
          build_tree_list(NULLT, build_unary_op(ADDR_EXPR,
		build_array_ref(_OBJC_SELECTOR_REFERENCES_decl,
				 build_int_2(selTransTbl_index,0)), 0)));
#else
  chainon(params,
          build_tree_list(NULLT,build_array_ref(_OBJC_SELECTOR_REFERENCES_decl,
    ...
    if (TREE_STATIC_TEMPLATE(basetype))
	  ...
	  if ((TREE_CODE(implementation_context) == IMPLEMENTATION_TYPE &&
	      ...
	      (TREE_CODE(implementation_context) == CATEGORY_TYPE &&
               implementation_context->class.super_name == TYPE_NAME(basetype)))
            return 1;
          }
...
tree continue_class(tree aClass)
{
  if (TREE_CODE(aClass) == IMPLEMENTATION_TYPE ||
    ...
    {
    tree record = xref_tag(RECORD_TYPE, aClass->class.my_name);

    if (!TYPE_FIELDS(record))
      {
      finish_struct(record, build_ivar_chain(aClass));
  ...
  if (method->method.sel_args)
      ...
      do 
	{
	  ...
	  TREE_OPERAND(last_expr,0) = arglist->keyword.arg_name;
          ...
          push_parm_decl(build_tree_list(arg_spec, arglist->keyword.arg_name));
