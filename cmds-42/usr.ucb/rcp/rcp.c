/* @(#)rcp.c	1.4 88/06/18 4.0NFSSRC SMI */
/*
...
#ifndef lint
static char sccsid[] = 	"@(#)rcp.c	1.4 88/06/18 4.0NFSSRC SMI"; /* from UCB 5.4 9/12/85 */
/* 
 * Upgraded to TAHOE
...
notreg:
		...
		if ((int)(bp = allocbuf(&buffer, f, BUFSIZ)) < 0) {
			(void) close(f);
			continue;
		}
		readerr = 0;
...
#define getnum(t) (t) = 0; while (isdigit(*cp)) (t) = (t) * 10 + (*cp++ - '0');
	...
	bad:
		...
		if (exists && pflag)
			(void) fchmod(of, mode);
		ga();
		if ((int)(bp = allocbuf(&buffer, of, BUFSIZ)) < 0) {
			(void) close(of);
			continue;
		}
		cp = bp->buf;
