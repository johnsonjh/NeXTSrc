...
static tree
require_instantiated_type (type, exp)
     tree type, exp;
{
  if (TREE_TYPE (exp) == unknown_type_node
      || (TREE_CODE (TREE_TYPE (exp)) == OFFSET_TYPE
	  && TREE_TYPE (TREE_TYPE (exp)) == unknown_type_node))
    return instantiate_type (type, exp, 1);
  return exp;
}
     ...
     int strict;
{
  ...
  switch (TREE_CODE (t1))
      ...
      if (strict <= 0)
	{
          ...
          if (doing_objc_thang)
            {
	    int objc_typchk = objc_comptypes(t1,t2);
     ...
     tree function, params, decl;
{
  ...
  if (is_method)
      ...
      else if (TREE_CODE (function) != IDENTIFIER_NODE)
	{
	  ...
	  function = build (OFFSET_REF, TREE_TYPE (type),
			    NULL_TREE, function);
	  goto do_x_function;
	}
  ...
  for (valtail = values, typetail = typelist;
	      ...
	      && TREE_TYPE (TREE_TYPE (val)) == unknown_type_node))
	{
	  ...
	  /* Convert FUNCTION_DECLs for virtual functions
	     to their proper representation.  */
	  type = TREE_TYPE (val);

	  if (TREE_CODE (type) == METHOD_TYPE)
	    basetype = TYPE_METHOD_BASETYPE (type);
	  ...
	  if (basetype)
	    {
	      val = build (OFFSET_REF, type,
			   build (NOP_EXPR, basetype, error_mark_node), val);
  ...
  switch (code)
      ...
      else if (code0 == POINTER_TYPE && code1 == POINTER_TYPE)
	{
	  ...
	  if (tt0 != tt1 && IS_AGGR_TYPE (tt0) && IS_AGGR_TYPE (tt1))
		...
		op0 = convert_pointer_to (tt1, op0);
	      else
		op1 = convert_pointer_to (tt0, op1);
	    }
     ...
     tree ifexp, op1, op2;
{
  ...
  op1 = require_instantiated_type (TREE_TYPE (op2), op1, 1);
  if (op1 == error_mark_node)
    return error_mark_node;
  op2 = require_instantiated_type (TREE_TYPE (op1), op2, 1);
	  ...
	  for (i = 1; i <= CLASSTYPE_N_BASECLASSES (lhstype); i++)
	    {
	      ...
	      /* Assignments fro virtual baseclasses handled elsewhere.  */
	      if (CLASSTYPE_VIA_VIRTUAL (lhstype, i))
		continue;

	      base_lhs = copy_node (lhs);
	      TREE_TYPE (base_lhs) = CLASSTYPE_BASECLASS (lhstype, i);
	      base_rhs = copy_node (newrhs);
	      TREE_TYPE (base_rhs) = TREE_TYPE (base_lhs);
     ...
     enum tree_code modifycode;
{
  ...
  rhs = require_instantiated_type (lhstype, newrhs, 1);
     ...
     char *errtype;
{
  ...
  /* Conversions among pointers */
  else if (codel == POINTER_TYPE && coder == POINTER_TYPE)
    {
      ...
      if (doing_objc_thang)
        {
        int objc_typchk = objc_comptypes(ttl,ttr);
     ...
     tree retval;
{
  ...
  if (retval == NULL_TREE)
	...
	retval = current_class_decl;
      else if (result != NULL_TREE && TREE_CODE (valtype) != VOID_TYPE)
	retval = result;
      else
	{
	  current_function_returns_null = 1;
