...
parse_spec(void)
{
	...
	if((spec_stream = fopen(spec_filename, "r")) == NULL)
	    perror_fatal("can't open: %s", spec_filename);
	...
	while(fgets(line, BUFSIZ, spec_stream) != NULL){
		...
		fatal("line %d in: %s too long (maximum %d)", line_num,
		...
		case STATE_UNDEFINED:
		    ...
		    break;
		default:
		    fatal("unrecognized specification in: %s on line %d",
	...
	if(ferror(spec_stream))
	    perror_fatal("read error on: %s", spec_filename);
	fclose(spec_stream);
	...
	for(i = 0; i <= max_slotnum_seen; i++){
		...
		error("branch table entry %d not specified in: %s ",
		      i, spec_filename);
		spec_error++;
	    }
	}
	...
	for(i = 0; i < branch_list_size; i++){
	    if(branch_list[i] != (struct branch *)0)
	    printf("\tbranch_list[%d] = 0x%x { %s, %d, 0x%x }\n", i,
		    branch_list[i],
		    branch_list[i]->name,
...
long oldstate)
{
	...
	if(strcmp(directive, "branch") == 0){
	     ...
	     */
	    if(*p != '\0')
		fatal("junk after #branch directive in: %s on line %d",
		      spec_filename, line_num);
	    if(branch_list != (struct branch **)0)
		fatal("#branch directive appears more that once in: %s on "
		      "line %d", spec_filename, line_num);
	    branch_list = (struct branch **)xmalloc(BRANCH_LIST_SIZE *
		...
		fatal("no object file name specified for #init directive in: "
		      "%s on line %d", spec_filename, line_num);
	    object_name = p;
	    p = next(p);
	    if(*p != '\0')
		fatal("junk after #init directive in: %s on line %d",
		      spec_filename, line_num);
	    if(object_hash == (struct object **)0){
		object_hash = (struct object **)xmalloc(OBJECT_HASH_SIZE *
			       sizeof(struct object *));
		...
		init_object = (struct object *)xmalloc(
				    sizeof(struct object));
	    ...
	    p = next(p);
	    if(*p != '\0')
		fatal("junk after #target directive in: %s on line %d",
		      spec_filename, line_num);
		...
		if(!isalnum(*p) && *p != '_' && *p != '.' && *p != '-')
		    fatal("base name of target: %s in: %s on line %d must have "
		...
		fatal("no segment name and address specified for #address "
		      "directive in: %s on line %d", spec_filename, line_num);
	    segment_name = p; 
	    p = next(p);
	    if(*p == '\0')
		fatal("missing address for #address directive in: %s on line "
		      "%d", spec_filename, line_num);
	    address_string = p;
	    p = next(p);
	    if(*p != '\0')
		fatal("junk after #address directive in: %s on line %d",
		      spec_filename, line_num);
	    address = strtoul(address_string, &q, 16);
	    if(*q != '\0')
		fatal("bad #address value: %s in: %s on line %d",
		      address_string, spec_filename, line_num);
	    if(address % getpagesize() != 0)
		fatal("#address value: 0x%x in: %s on line %d must be a "
		      "multiple of the system pagesize (0x%x)",
			address, spec_filename, line_num, getpagesize());
		...
		fatal("minor_version specified more than once (in the file: %s "
		      "on the line %d and on the command line or previouly in "
	    ...
	    p = next(p);
	    if(*p != '\0')
		fatal("junk after #minor_version directive in: %s on line %d",
		      spec_filename, line_num);
	    minor_version = atol(minor_version_string);
	    if(minor_version <= 0)
		fatal("#minor_version value: %d in: %s on line %d must be "
	     ...
	     */
	    if(*p != '\0')
		fatal("junk after #alias directive in: %s on line %d",
		      spec_filename, line_num);
	    return(STATE_ALIAS);
		...
		oddball_list = (struct oddball **)xmalloc(ODDBALL_LIST_SIZE *
	...
	fatal("unknown directive: #%s in: %s on line: %d", directive,
	      spec_filename, line_num);
	return(STATE_NONE);
}
...
char *p)
{
	...
	p = next(p);
	if(*p == '\0')
	    fatal("missing branch table possition number in: %s on line %d",
		  spec_filename, line_num);
	min_slotnum = strtol(p, &q, 10);
	if(q == p)
	    fatal("bad branch table possition number in: %s on line %d",
		  spec_filename, line_num);
	if(min_slotnum < 0)
	    fatal("branch table possition number in: %s on line %d must be "
		  "greater than zero", spec_filename, line_num);
	p = q;
	while(isspace(*p))
	    p++;
	if(*p != '\0' && *p != '#' && *p != '-')
	    fatal("bad branch table specification in: %s on line %d",
		  spec_filename, line_num);
	if(*p == '-'){
	    p++; /* skip the '-' */
	    while(isspace(*p))
	    ...
	    if(q == p)
		fatal("bad upper branch table possition range in: %s on ",
		      spec_filename, line_num);
	    if(max_slotnum < min_slotnum)
		fatal("bad upper branch table possition range (%d) in: %s on "
		      "line %d must be greater than lower (%d)", max_slotnum,
		       spec_filename, line_num, min_slotnum);
	    while(isspace(*q))
		q++;
	    if(*q != '\0' && *q != '#')
		fatal("junk after #branch specification in: %s on line %d",
		      spec_filename, line_num);
	}
	else{
	...
	if(bp == (struct branch *)0){
	    bp = (struct branch *)xmalloc(sizeof(struct branch));
	...
	if(max_slotnum >= branch_list_size){
	    branch_list = (struct branch **)xrealloc(branch_list,
			   max_slotnum * 2 *
			   sizeof(struct branch *));
	...
	for(i = min_slotnum; i <= max_slotnum; i++){
	    if(branch_list[i] != (struct branch *)0 &&
	       branch_list[i] != bp){
		error("#branch position %d multiply specified in: %s on line "
		      "%d", i, spec_filename, line_num);
		spec_error++;
	    }
	    else{
		if(branch_list[i] != bp)
 ...
 * space.  For each of them a object structure is found (as previouly
 * entered in the hash table) or created and added to the object_list.  Also
 * it is entered in hash table if it was not found there.
 */
static
void
object(
char *p)
{
	...
	while(*object_name != '\0'){
		...
		object_list = (struct object **)xrealloc(object_list,
			       object_list_size * 2 *
			       sizeof(struct object *));
		object_list_size *= 2;
	    }
		...
		if(strcmp(op->base_name, base_name) == 0){
			...
			fatal("object files: %s and %s in: %s must have unique "
			      "base file names (%s)", op->name, object_name,
			      spec_filename, base_name);
		    }
		    if(op->init_only)
			break;
		    fatal("object file name: %s appears more than once in: %s "
			  "on line %d", object_name, spec_filename, line_num);
		}
		op = op->next;
	    }
	    p = base_name;
	    while(*p != '\0'){
		if(!isalnum(*p) && *p != '_' && *p != '.' && *p != '-')
		    fatal("base name of object: %s in: %s on line %d must have "
			  "only alphanumeric, '_', '-', or '.' characters in "
			  "it", object_name, spec_filename, line_num);
		p++;
	    }
	    if(op == (struct object *)0){
		op = (struct object *)xmalloc(sizeof(struct object));
...
char *p)
{
	...
	p = next(p);
	if(*p != '\0')
	    fatal("junk after #init specification in: %s on line %d",
		  spec_filename, line_num);
	ip = (struct init *)xmalloc(sizeof(struct init));
...
char *p)
{
	...
	p = next(p);
	if(*p != '\0')
	    fatal("junk after #alias specification in: %s on line %d",
		  spec_filename, line_num);
	ap = (struct alias *)xmalloc(sizeof(struct alias));
...
long state)
{
	...
	while(*name != '\0'){
		...
		oddball_list = (struct oddball **)xrealloc(oddball_list,
			       oddball_list_size * 2 *
			       sizeof(struct oddball *));
		oddball_list_size *= 2;
	    }
		...
		obp = (struct oddball *)xmalloc(sizeof(struct oddball));
...
#ifdef 0
/*
 * nobranch is called after a #nobranch_text directive has been seen and we
 * are in nobranch state.  The names of external symbols then appear separated
 * by white space.  For each of them an oddball structure is found (as previouly
 * entered in the hash table) or created and added to the oddball_list.  Also
 * it is entered in hash table if it was not found there.  Then the oddball
 * structure is marked nobranch.
 */
static
void
nobranch(p)
