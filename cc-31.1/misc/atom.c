...
/*
 * These can't really be prototyped because of their variable number and types
 * of arguments.
 */
void error(/* char *, va_list */);
void fatal(/* char *, va_list */);
void fatal_perror(/* char *, va_list */);

void *xmalloc(long);
void *xrealloc(void *, long);
long round(long v, unsigned long r);
...
struct nlist *lookup(char *sym_name);
    ...
    char *envp[])
{
	...
	long i, j, k, len, header_size, vmaddr, fileoff, s_addr, s_offset,
	     page_size;
	char *p;
	int fd;
	...
	for(i = 1; i < argc; i++){
		...
		if(i + 3 >= argc)
		    fatal("missing arguments to -segcreate <segment name> "
			  "<section name> <file name>", progname);
		if(stat(argv[i+3], &statbuf) == -1)
		    fatal_perror("Can't stat file: %s (to create a segment "
				 "with)", argv[i+3]);
	...
	if((afd = open(aoutfile, O_RDONLY)) == -1)
	    fatal_perror("Can't open a.out file: %s", aoutfile);
	if(read(afd, &exec, sizeof(struct exec)) != sizeof(struct exec))
	    fatal_perror("Can't read exec header of a.out file: %s", aoutfile);
	...
	if((mfd = open(machofile, O_WRONLY|O_CREAT|O_TRUNC, 0)) == -1)
	    fatal_perror("Can't create Mach-O file: %s", machofile);
	...
	text_segment.filesize = exec.a_text;
	...
	if(objcflag){
	     ...
	     */
	    symtab = (struct nlist *)xmalloc(exec.a_syms);
	    lseek(afd, N_SYMOFF(exec), L_SET);
	    if(read(afd, symtab, exec.a_syms) != exec.a_syms)
		fatal_perror("Can't read symbol table from a.out file: %s",
			     aoutfile);
	    lseek(afd, N_STROFF(exec), L_SET);
	    if(read(afd, &strsize, sizeof(long)) != sizeof(long))
		fatal_perror("Can't read string table size from a.out file: "
			     "%s", aoutfile);
	    strtab = (char *)xmalloc(strsize);
	    lseek(afd, N_STROFF(exec), L_SET);
	    if(read(afd, strtab, strsize) != strsize)
		fatal_perror("Can't read string table from a.out file: %s",
			     aoutfile);
		...
		if(read(afd, data_reloc, exec.a_drsize) != exec.a_drsize)
		    fatal_perror("Can't read data relocation entries from "
	...
	if(fstat(afd, &statbuf) == -1)
	    fatal_perror("Can't stat a.out file: %s ", aoutfile);
	...
	if(exec.a_syms != 0){
		...
		lseek(afd, N_STROFF(exec), L_SET);
		if(read(afd, &strsize, sizeof(long)) != sizeof(long))
		    fatal_perror("Can't read string table size from a.out "
				 "file : %s", aoutfile);
	    }
	...
	if(write(mfd, &header, sizeof(struct mach_header)) !=
	    ...
	    fatal_perror("Can't write mach header to Mach-O file: %s",
			 machofile);
	if(exec.a_magic == ZMAGIC){
	    if(write(mfd, &pagezero_segment, sizeof(struct segment_command)) !=
		    sizeof(struct segment_command))
		fatal_perror("Can't write segment_command for: %s segment to "
	    ...
	    if(write(mfd, &text_section, sizeof(struct section)) !=
		    sizeof(struct section))
		fatal_perror("Can't write section header for: %s section to "
	    ...
	    if(write(mfd, &symbol_table, sizeof(struct symtab_command)) !=
		    sizeof(struct symtab_command))
		fatal_perror("Can't write symtab_command to "
			     "Mach-O file: %s", machofile);
	    if(ggflag)
		if(write(mfd, &symbol_segment, sizeof(struct symseg_command)) !=
			sizeof(struct symseg_command))
		    fatal_perror("Can't write symseg_command to "
				 "Mach-O file: %s", machofile);
	    if(write(mfd, &thread, sizeof(struct thread_command)) !=
		    sizeof(struct thread_command))
		fatal_perror("Can't write thread_command to "
			     "Mach-O file: %s", machofile);
	    if(write(mfd, &flavor, sizeof(unsigned long)) !=
		    sizeof(unsigned long))
		fatal_perror("Can't write thread flavor to "
			     "Mach-O file: %s", machofile);
	    if(write(mfd, &count, sizeof(unsigned long)) !=
		    sizeof(unsigned long))
		fatal_perror("Can't write thread count to "
			     "Mach-O file: %s", machofile);
	    if(write(mfd, &cpu, sizeof(struct NeXT_thread_state_regs)) !=
		    sizeof(struct NeXT_thread_state_regs))
		fatal_perror("Can't write thread state to "
			     "Mach-O file: %s", machofile);
	}
	else{
	    if(write(mfd, &reloc_segment, sizeof(struct segment_command)) !=
		    sizeof(struct segment_command))
		fatal_perror("Can't write segment_command to "
			     "Mach-O file: %s", machofile);
	    if(write(mfd, &text_section, sizeof(struct section)) !=
		    sizeof(struct section))
		fatal_perror("Can't write section header for: %s section to "
		...
		if(write(mfd, &sym_section, sizeof(struct section)) !=
			sizeof(struct section))
		    fatal_perror("Can't write section header for: %s section "
	...
	for(i = 0; i < n_seg_create; i++){
	    if(write(mfd, &(seg_create[i].sg), sizeof(struct segment_command))
	       != sizeof(struct segment_command))
		fatal_perror("Can't write load commands for segments created "
			     "from files to Mach-O file: %s", machofile);
	    slp = seg_create[i].slp;
	    for(j = 0; j < seg_create[i].sg.nsects; j++){
		if(write(mfd, &(slp->s), sizeof(struct section))
		   != sizeof(struct section))
		    fatal_perror("Can't write section structures of the load "
	...
	text = (char *)xmalloc(exec.a_text);
	lseek(afd, N_TXTOFF(exec), L_SET);
	if(read(afd, text, exec.a_text) != exec.a_text)
	    fatal_perror("Can't read text from a.out file: %s", aoutfile);
	if(write(mfd, text, exec.a_text) != exec.a_text)
	    fatal_perror("Can't write text to Mach-O file: %s", machofile);

	/* read and write the data */
	data = (char *)xmalloc(exec.a_data);
	if(read(afd, data, exec.a_data) != exec.a_data)
	    fatal_perror("Can't read data from a.out file: %s", aoutfile);
	if(write(mfd, data, exec.a_data) != exec.a_data)
	    fatal_perror("Can't write data to Mach-O file: %s", machofile);

	/* read and write the segments created from files */
	for(i = 0; i < n_seg_create; i++){
	    slp = seg_create[i].slp;
	    for(j = 0 ; j < seg_create[i].sg.nsects ; j++){
		p = (char *)xmalloc(slp->filesize);
		if((fd = open(slp->filename, O_RDONLY)) == -1)
		    fatal_perror("Can't open file: %s to create section %s in "
			         "segment: %s", slp->filename, slp->s.sectname,
				 slp->s.segname);
		if(read(fd, p, slp->filesize) != slp->filesize)
		    fatal_perror("Can't read file: %s to create section %s in "
			         "segment: %s", slp->filename, slp->s.sectname,
				 slp->s.segname);
		lseek(mfd, slp->s.offset, L_SET);
		if(write(mfd, p, slp->filesize) != slp->filesize)
		    fatal_perror("Can't write contents of: %s to Mach-O file: "
	...
	if(exec.a_trsize != 0){
	    p = (char *)xmalloc(exec.a_trsize);
	    if(read(afd, p, exec.a_trsize) != exec.a_trsize)
		fatal_perror("Can't read text relocation entries from a.out "
			     "file: %s", aoutfile);
		...
		if(preloc->r_extern == 0){
			...
			fatal("Bad r_symbolnum field (0x%x) for a local "
			      "relocation entry (%d)", preloc->r_symbolnum,  
			      (struct relocation_info *)p - preloc);
		    }
		}
	    }
		...
		fatal_perror("Can't write text relocation entries to Mach-O "
			     "file: %s", machofile);
	    free(p);
	}
	...
	if(exec.a_drsize != 0){
		...
		fatal_perror("Can't write data relocation entries to Mach-O "
			     "file: %s", machofile);
	    free(p);
	}
	...
	if(exec.a_syms != 0){
		...
		fatal_perror("Can't write symbol table from a.out file: %s",
			     aoutfile);
	    free(p);
	}
	...
	if(strsize != 0){
		...
		fatal_perror("Can't write string table from a.out file: %s",
			     aoutfile);
	    free(p);
	}
	...
	if(ggflag && symbol_segment.size != 0){
	    p = (char *)xmalloc(symbol_segment.size);
	    if(read(afd, p, symbol_segment.size) != symbol_segment.size)
		fatal_perror("Can't read symbol segment from a.out file: %s",
			     aoutfile);
	    /*
		...
		*pmach_root = mach_root;
		...
		fatal_perror("Can't write symbol segment to Mach-O file: %s",
			     machofile);
	    free(p);
	}
	...
	if(fchmod(mfd, statbuf.st_mode & 0777) == -1)
	    fatal_perror("Can't change mode of Mach-O file: %s", machofile);
...
long size;
{
	...
	if((p = malloc(size)) == NULL)
	    fatal_perror("virtual memory exhausted");
	return(p);
}
