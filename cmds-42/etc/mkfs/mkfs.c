...
#ifndef lint
static char sccsid[] = "@(#)mkfs.c    1.2 88/03/10 4.0NFSSRC; from    5.3 (Berkeley) 9/11/85";
#endif not lint
...
next:
	...
	if (sblock.fs_ipg > MAXIPG) {
		...
		printf("Warning: %d bytes per inode impossible due\n", nbpi);
		printf("to cylinder group size, using %d bytes per inode\n",
