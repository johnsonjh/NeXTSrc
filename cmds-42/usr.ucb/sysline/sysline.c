 ...
 *   -P		Append PID when looking for .who and .syslinelock
 */
...
int ls_cap = 0;		/* set to 1 if termcap 'ls' capability used */
	...
	register char **argv;
{
	...
	if (usepids)
		...
		pidstr[0] = '\0';
	strcpy1(strcpy1(strcpy1(whofilename, home), "/.who"), pidstr);
	strcpy1(strcpy1(strcpy1(whofilename2, home), "/.sysline"), pidstr);
	strcpy1(strcpy1(strcpy1(lockfilename, home), "/.syslinelock"), pidstr);
#else XHACKS
...
#ifdef VMUNIX
#if NeXT
		table(TBL_LOADAVG, 0, &tl, 1, sizeof(tl));
		if (tl.tl_lscale != 0) {
			for (i = 0; i < 3; i++)
				avenrun[i] = (float)tl.tl_avenrun[i]/(float)tl.tl_lscale;
		} else {
			for (i = 0; i < 3; i++)
				avenrun[i] = *(float *)&tl.tl_avenrun[i];
		}
#else
 ...
 * if the columns were obtained by using the 'ls' termcap value, so that
