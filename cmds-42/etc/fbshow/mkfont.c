/*
 * mkfont ASCII_BITMAP_INPUT [BINARY_BITMAP_OUTPUT]
 *
 * This program parses the ascii bitmap (screen font) description given as
 * input and produces a binary font file suitable for use with "newblit".
 * If the output file is not specified, it is created in the current directory
 * and given the name FontFamily-FontFace.PointSize; e.g. for 18 point
 * Helvetica Bold, the output file would be: Helvetica-Bold.18.
 */
...
main(int argc, const char * const argv[])
{
	...
	if (argc < 1 || argc > 2)
		fatal("Usage: %s ASCII_BITMAP_INPUT [BINARY_BITMAP_OUTPUT]");
	 ...
	 && getline(linebuf, sizeof linebuf, input) != NULL) {
		...
		case FONTBOUNDINGBOX_LINE:
			if (sscanf(linebuf, "%*s %hd %hd %hd %hd",
			 &font.bbx.width, &font.bbx.height, &font.bbx.xoff, &font.bbx.yoff) != 4)
				parse_error();
...
read_char_description(void)
{
	...
	while (! did_endchar_line && getline(linebuf, sizeof linebuf, input) != NULL) {
		...
		case ENDCHAR_LINE:
			...
			break;
		case DWIDTH_LINE:
			if (sscanf(linebuf, "%*s %hd", &bm.dwidth) != 1)
				parse_error();
			if (did_dwidth_line)
				duplicate("DWIDTH");
		...
		case BBX_LINE:
			if (sscanf(linebuf, "%*s %hd %hd %hd %hd",
			 &bm.bbx.width, &bm.bbx.height, &bm.bbx.xoff, &bm.bbx.yoff) != 4)
			 	parse_error();
			if (did_bbx_line)
				duplicate("BBX");
			did_bbx_line = TRUE;
...
parse_error(void)
{
	fatal("Couldn't parse: %s", linebuf);
}
