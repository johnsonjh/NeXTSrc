...
char	temp_pag[] = PWPATH(ptmp.pag);
char	temp_dir[] = PWPATH(ptmp.dir);
char	passwd[] = PWPATH(passwd);
char	passwd_pag[] = PWPATH(passwd.pag);
char	passwd_dir[] = PWPATH(passwd.dir);
char	buf[BUFSIZ];
char	*getenv();
char	*index();
extern	int errno;
	...
	char *argv[];
{
	...
	if (system(buf) == 0) {
		...
		while (fgets(buf, sizeof (buf) - 1, ft) != NULL) {
			register char *cp, *home;
			struct stat statb;

			/*
			 * Look for a valid root password entry
			 */
			cp = index(buf, '\n');
			if (cp == 0)
				continue;
			...
			*cp = '\0';
			...
			if (cp == 0) {
trunc:
				fprintf(stderr, "vipw: root entry incomplete\n");
				break;
			}
			/* uid */
			if (atoi(cp + 1) != 0) {
				fprintf(stderr, "vipw: root entry has non-zero uid\n");
				break;
			}
			cp = index(cp + 1, ':');
			if (cp == 0)
			...
			if (cp != 0)
				*cp = '\0';
			if (stat(home, &statb) < 0 ||
			     (statb.st_mode & S_IFMT) != S_IFDIR) {
				fprintf(stderr, "vipw: home directory does not exist for root\n");
				break;
			}
			if (cp && *(cp+1) && strcmp(cp+1, "/bin/sh") &&
			    strcmp(cp+1, "/bin/csh")) {
				fprintf(stderr, "vipw: invalid shell for root\n");
				break;
			}
			ok++;
		}
	...
	char *file;
{
	...
	if ((pid = vfork()) == 0) {
		execl("/usr/etc/mkpasswd", "mkpasswd", file, 0);
		_exit(127);
	}
