...
int debug  = 0;
char f1mark[78], f3mark[78]; /*markers for -E and -X*/
...
char **argv;
{
	...
	if(argc<6) {
        ...
        if (oflag) { 
		strcpy(f1mark, "<<<<<<< ");
		makemark(f1mark, argc >= 7 ? argv[6] : argv[3], sizeof(f1mark));
		strcpy(f3mark, ">>>>>>> ");
		makemark(f3mark, argc >= 8 ? argv[7] : argv[5], sizeof(f3mark));
        }
...
void
makemark(mark, file, marksize)
char *mark, *file;
int marksize;
{
	register int filelen, marklen;
	register char *cp;

	filelen = strlen(file);
	marklen = strlen(mark);
	if (marklen + filelen >= marksize) {
		file += filelen - (marksize - marklen) + 4;
		if ((cp = index(file, '/')) != NULL)
			file = cp;
		strcat(mark, "...");
		strcat(mark, file);
	} else
